...
with tab_monthly:
    ...
    if monthly_entries:
        ...
        # Arşive de kaydedelim (tekil olarak isme göre)
        st.session_state.pdf_archive[monthly_pdf_filename] = base64.b64encode(m_pdf_bytes).decode('utf-8')
        db_save("pdf_archive", st.session_state.pdf_archive)

        st.download_button(
            label=f"📥 {months_dict[sel_m_month_num]} {sel_m_year} Aylık Raporu PDF Olarak İndir",
            data=m_pdf_bytes,
...
with tab_archive:
    ...
    if st.session_state.pdf_archive:
        # Benzersiz hale getirme ve düzenleme
        clean_archive = {}
    ...
    if st.button("📄 Bu Haftanın PDF'lerini Arşive Kaydet / Güncelle", type="primary"):
        if active_projects:
            for proj in active_projects:
                ...
                # Proje kodunu ayıkla veya temiz dosya adı oluştur
...
with col_act2:
    if is_week_locked:
        ...
        if st.button("✅ Haftayı Tamamla ve Kilitle"):
            ...
            db_save("completed_weeks", list(st.session_state.completed_weeks))
            st.success("Hafta başarıyla kilitlendi ve güvene alındı! (🔴 Kırmızı nokta aktif)")
            st.rerun()

# Aktif projelerin indirme butonları
if active_projects:
    st.markdown("#### Aktif Proje PDF İndirme Bağlantıları:")
    for proj in active_projects:
        pdf_file = create_pdf(proj, all_entries, all_expenses, week_no, sel_year)
        pdf_bytes = pdf_file.getvalue()
        clean_proj_name = proj.replace(' ', '_').replace('(', '').replace(')', '')
