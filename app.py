...
# --- SLIDER SAAT SEÇİCİ (MAVİ ÇİZGİ HİZASI: [3, 1] ORANI) ---
def time_picker_slider_component(label, min_h, max_h, session_key, default_val, disabled=False):
    ...
    if default_val not in time_options:
        default_val = f"{min_h:02d}:00"
        
    slider_key = f"slider_time_{session_key}"
    if slider_key not in st.session_state or st.session_state[slider_key] not in time_options:
        st.session_state[slider_key] = default_val
        
    # [3, 1] oranı ile slider mavi çizgiye kadar uzatıldı, sağ taraf güvenli kaydırma alanı oldu
    ...
    with col2:
        st.markdown(f"<div style='padding-top: 6px; font-size: 11px; font-weight: bold; color: #2563EB;'>{selected_time}</div>", unsafe_allow_html=True)
...
def break_picker_component(label, session_key, default_val, disabled=False):
    ...
    with col2:
        st.markdown(f"<div style='padding-top: 6px; font-size: 11px; font-weight: bold; color: #2563EB;'>{sel_break}</div>", unsafe_allow_html=True)
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                final_start = time_picker_slider_component("Başlangıç (03:00 - 12:00)", 3, 12, f"s_{day}{p_i}{w_i}_{week_no}", st.session_state.get(s_key, def_s), disabled=is_week_locked)
                                ...
                                final_stop = time_picker_slider_component("Bitiş (11:00 - 20:00)", 11, 20, f"e_{day}{p_i}{w_i}_{week_no}", st.session_state.get(e_key, def_e), disabled=is_week_locked)
...
@media (max-width: 768px) {
    ...
    /* Form elemanlarını da biraz kısalt */
    ...
    /* Selectbox içindeki gerçek kutu */
