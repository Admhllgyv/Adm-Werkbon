...
st.markdown("""
    ...
    /* Pills stilini mobil için daha sıkı hale getiriyoruz */
...
# Saat seçenekleri (Pills için)
start_slots = ["05:00", "05:30", "06:00", "06:30", "07:00", "07:30", "08:00", "08:30", "09:00", "Manuel"]
end_slots = ["14:00", "14:30", "15:00", "15:30", "15:45", "16:00", "16:15", "16:30", "17:00", "17:15", "17:30", "Manuel"]
pause_slots = ["00:00", "00:15", "00:30", "00:45", "01:00", "01:30", "01:45", "Manuel"]
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                # Sadece Çalışan Seçimi
                wrk = st.selectbox(f"Çalışan {i+1} ({day})", w_list, key=w_key)
                
                # Klavyeyi engellemek için KLAVYEYİ ASLA AÇMAYAN PİLLS KULLANIMI
                st.caption(f"⏱️ Saat Ayarları - Çalışan {i+1}")
                
                # Başlangıç
                s_val = saved_e.get("Başlangıç", "07:00")
                if wrk == "Adem" and not d_entries: s_val = "05:30"
                s_choice = st.pills(f"Başlangıç {i+1}", start_slots, default=s_val if s_val in start_slots else "Manuel", key=f"s_pills_{day}_{i}_{week_no}")
                start = st.text_input(f"Manuel Başlangıç {i+1}", value=s_val, key=f"s_man_{day}_{i}_{week_no}") if s_choice == "Manuel" else (s_choice if s_choice else s_val)

                # Mola
                p_val = saved_e.get("Mola", "00:45")
                if wrk == "Adem" and not d_entries: p_val = "01:45"
                p_choice = st.pills(f"Mola {i+1}", pause_slots, default=p_val if p_val in pause_slots else "Manuel", key=f"p_pills_{day}_{i}_{week_no}")
                pause = st.text_input(f"Manuel Mola {i+1}", value=p_val, key=f"p_man_{day}_{i}_{week_no}") if p_choice == "Manuel" else (p_choice if p_choice else p_val)

                # Bitiş
                e_val = saved_e.get("Bitiş", "15:45")
                if wrk == "Adem" and not d_entries: e_val = "17:15"
                e_choice = st.pills(f"Bitiş {i+1}", end_slots, default=e_val if e_val in end_slots else "Manuel", key=f"e_pills_{day}_{i}_{week_no}")
                stop = st.text_input(f"Manuel Bitiş {i+1}", value=e_val, key=f"e_man_{day}_{i}_{week_no}") if e_choice == "Manuel" else (e_choice if e_choice else e_val)
                
                calc_hours = calculate_net_hours(start, pause, stop)
                st.markdown(f"**Net Çalışma:** <span style='color:blue'>{calc_hours} saat</span>", unsafe_allow_html=True)
