...
# 03:00 - 22:00 arası 15'er dakikalık aralıklar
...
# Sekmeler (Aylık Rapor Sekmesi Eklendi)
...
with tab_archive:
    ...
    st.caption("Telefonunuzun kendi PDF programıyla açmak veya indirmek için butonları kullanabilirsiniz.")
    if st.session_state.pdf_archive:
        for filename, b64_pdf in list(st.session_state.pdf_archive.items()):
            ...
            # Doğrudan telefon / tarayıcı içi önizleme için embed alanı
...
with tab_monthly:
    st.header("📅 Aylık Rapor ve Detaylı Analiz")
    ...
    # Tüm haftalık verilerden seçilen aya ait kayıtları topla
    monthly_entries = []
    monthly_expenses = []
    ...
    if monthly_entries:
        ...
        st.info(f"{sel_month_name} {sel_month_year} dönemi için henüz kayıt bulunmuyor.")
