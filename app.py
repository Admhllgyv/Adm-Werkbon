...
# --- AKILLI VE KLAVYE TETİKlemeyen SABİT SAAT SEÇİCİ ---
def time_picker_component_unified(label, min_h, max_h, session_key, default_val, disabled=False):
    ...
    col_lbl, col_chk = st.columns([3, 2])
    with col_chk:
        is_manual = st.checkbox("✍️ Manuel Yaz", key=f"chk_{man_key}", disabled=disabled)
    ...
    if is_manual:
        ...
        sel_time = st.selectbox(label, time_options, key=t_key, disabled=disabled, label_visibility="collapsed")
        return sel_time
        ...
        with st.expander(expand_label, expanded=(len(day_entries) > 0)):
            ...
            if num_projects > 0:
                ...
                for p_i in range(int(num_projects)):
                    ...
                    day_expenses = [ex for ex in saved_expenses_by_day.get(day, []) if ex.get("Proje"] == proj_sel] # fixed bracket issue if any, wait, ex.get("Proje") == proj_sel
