...
# Saat Slotu Üreteçleri (İstediğin Aralıklar)
...
start_slots = generate_slots(3, 30, 10, 30) # 03:30 - 10:30 (15 dk aralıkla)
end_slots = generate_slots(12, 30, 19, 30)   # 12:30 - 19:30 (15 dk aralıkla)
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                # Çalışan listesi boşluk ("Seçiniz") ile başlar
                worker_options = ["Seçiniz"] + st.session_state.workers
                
                if w_key not in st.session_state:
                    st.session_state[w_key] = saved_e.get("Çalışan", "Seçiniz")
                
                current_worker = st.session_state.get(w_key, "Seçiniz")
                if current_worker not in worker_options:
                    current_worker = "Seçiniz"
                
                w_idx = worker_options.index(current_worker)
                wrk = st.selectbox(f"Çalışan {i+1} ({day})", worker_options, index=w_idx, key=w_key)
                
                # Çalışana göre otomatik saat belirleme
                if wrk == "Adem":
                ...
                if p_key not in st.session_state:
                    st.session_state[p_key] = saved_e.get("Mola", default_p)
                ...
                if man_p:
                    ...
                    curr_p = st.session_state.get(p_key, default_p)
