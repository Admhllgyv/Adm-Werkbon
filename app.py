...
tab_entry, tab_projects, tab_archive, tab_preview, tab_settings = st.tabs(["📝 Werkbon Girişi", "📁 Proje Yönetimi", "📚 PDF Arşivi", "👀 Önizleme", "⚙️ Ayarlar"])
...
with tab_archive:
    ...
    st.markdown("Ürettiğiniz tüm PDF raporları burada otomatik olarak saklanır. İstediğiniz zaman bu sabit arşivden PDF'lerinizi tekrar indirebilirsiniz.")
    ...
    if st.session_state.pdf_archive:
        ...
        st.info("Henüz arşive kaydedilmiş bir PDF bulunmuyor. Önizleme sekmesinden PDF ürettiğinizde buraya otomatik eklenecektir.")
...
with tab_entry:
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=(day == "Pazartesi")):
...
with tab_preview:
    ...
    if all_entries:
        ...
        for proj in unique_projects:
            ...
            st.dataframe(pd.DataFrame(proj_e)[["Gün", "Tarih", "Çalışan", "Başlangıç", "Mola", "Bitiş", "Saat"]])
            
            if proj_ex:
                st.write("**Gider Detayları:**")
                st.dataframe(pd.DataFrame(proj_ex)[["Gün", "Tarih", "Kod", "Açıklama", "Tutar"]])
            st.markdown("---")
    else:
...
st.markdown("---")
st.subheader("📥 Raporu PDF Olarak İndir ve Arşivle (Nederlands)")
    ...
    for proj in unique_projects:
        ...
        with col_dl2:
            if current_week_key in st.session_state.completed_weeks:
                if st.button(f"🔄 Tamam Değil Yap", key=f"unmark_{proj}_{week_no}"):
                    st.session_state.completed_weeks.remove(current_week_key)
                    db_save("completed_weeks", list(st.session_state.completed_weeks))
                    st.success("Hafta tekrar aktif yapıldı!")
                    st.rerun()
            else:
