...
# Sekmeler
tab_entry, tab_projects, tab_archive, tab_settings = st.tabs(["📝 Werkbon Girişi", "📁 Proje Yönetimi", "📚 PDF Arşivi", "⚙️ Ayarlar"])
...
with tab_entry:
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            # DÜZELTME: Her gün için ana bir proje seçimi tanımlıyoruz ki o günkü tüm saatler ve fişler kesinlikle bu projeye yazılsın
            day_proj = st.selectbox(f"📌 {day} Projesi", project_options, key=f"day_proj_{day}_{week_no}")

            num_rows = st.number_input(f"{day} çalışma satırı sayısı", min_value=0, max_value=10, value=0, key=f"num_{day}_{week_no}")
            ...
            for j in range(int(num_exp)):
                ...
                if exp_amount > 0 and day_proj:
                    ...
                    all_expenses.append({
                        "Gün": day,
                        "Tarih": day_date_str,
                        "Proje": day_proj, # DÜZELTME: Doğrudan günün projesiyle eşleştirildi, böylece Salı günkü fişler kaçmaz!
                        "Kod": doc_code,
                        "Açıklama": exp_desc,
                        "Tutar": exp_amount,
    ...
    if all_entries or all_expenses:
        ...
        if st.session_state.get(f"pdf_generated_{week_no}", False):
            ...
            # Tüm girilen proje adlarını (hem çalışmalardan hem giderlerden) topluyoruz ki sadece saat girilmeyip fiş girilen projeler de atlanmasın
