...
st.markdown("""
    ...
    /* Tabloların telefonda sağa sola kaymasını önleyen, ekrana tam sığdıran stil */
    .resp-table {
        width: 100% !important;
...
with tab_settings:
    ...
    try:
        ...
        st.info("Excel raporu hazırlanırken bir kütüphane eksikliği oluştuysa CSV olarak da indirebilirsiniz:")
        if all_weeks_entries:
...
with tab_entry:
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                with cols[0]:
                    wrk = st.selectbox("Çalışan", w_list, key=w_key, on_change=update_worker_defaults, args=(day, i, week_no))
                with cols[1]:
                    s_choice = st.selectbox("Başlangıç", start_slots, key=s_choice_key)
                    start = st.text_input("Örn: 07:00", value=st.session_state.get(f"s_man_{day}_{i}_{week_no}", s_choice if s_choice != "Manuel Giriş" else "07:00"), key=f"s_man_{day}_{i}_{week_no}") if s_choice == "Manuel Giriş" else s_choice
                with cols[2]:
                    p_choice = st.selectbox("Mola", pause_slots, key=p_choice_key)
                    pause = st.text_input("Örn: 00:45", value=st.session_state.get(f"p_man_{day}_{i}_{week_no}", p_choice if p_choice != "Manuel Giriş" else "00:45"), key=f"p_man_{day}_{i}_{week_no}") if p_choice == "Manuel Giriş" else p_choice
                with cols[3]:
                    e_choice = st.selectbox("Bitiş", end_slots, key=e_choice_key)
                    stop = st.text_input("Örn: 15:45", value=st.session_state.get(f"e_man_{day}_{i}_{week_no}", e_choice if e_choice != "Manuel Giriş" else "15:45"), key=f"e_man_{day}_{i}_{week_no}") if e_choice == "Manuel Giriş" else e_choice
    ...
    def create_pdf(project_name, entries, expenses, week, year):
        ...
        sub_style = ParagraphStyle('SubStyle', fontName='Helvetica', fontSize=8, textColor=text_dark)
        bold_sub_style = ParagraphStyle('BoldSubStyle', fontName='Helvetica-Bold', fontSize=8, textColor=primary_color)
        ...
        if logo_io is not None:
            try:
                ...
                img_io.seek(0)
                rl_img = RLImage(img_io, width=42, height=42)
                header_left_elements.append(rl_img)
            except Exception:
                pass
        
        header_text = f"<b><font size=11 color='#0F172A'>{st.session_state.comp_name}</font></b><br/><font size=7 color='#64748B'>{st.session_state.comp_address}<br/>BTW: {st.session_state.comp_btw}</font>"
        header_left_elements.append(Paragraph(header_text, sub_style))
        
        meta_p_style = ParagraphStyle('MetaRight', fontName='Helvetica', fontSize=8, textColor=text_dark, alignment=2)
        header_data = [
            [
                header_left_elements,
        ...
        t_header = Table(header_data, colWidths=[260, 275])
        ...
        # --- ÇALIŞAN BAZLI SIRALAMA GÜNCELLEMESİ ---
        day_order = {"Pazartesi": 1, "Salı": 2, "Çarşamba": 3, "Perşembe": 4, "Cuma": 5, "Cumartesi": 6, "Zondag": 7}
        # Önce çalışana göre, sonra gün sırasına göre sıralıyoruz (Adem önce, sonra Melih vb.)
        ...
        t_main = Table(table_content, colWidths=[90, 110, 65, 65, 65, 140])
        ...
        if proj_expenses:
            ...
            for ex in proj_expenses:
                ...
                total_cost += ex["Tutar"]
            t_exp = Table(exp_table_content, colWidths=[80, 95, 260, 100])
