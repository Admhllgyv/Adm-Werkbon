...
/* Ekranın bütünüyle kaymasını önleyip sadece radyo buton konteynerini kaydırılabilir yapma */
...
<script>
// Seçili olan radio butonun her zaman yatay ortada kalmasını sağlayan otomatik odaklama fonksiyonu
function centerSelectedRadios() {
    const checkedInputs = document.querySelectorAll('div[data-testid="stRadio"] input[type="radio"]:checked');
    checkedInputs.forEach(input => {
        const label = input.closest('label');
        if (label) {
            label.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
        }
    });
}
// Sayfa yüklendiğinde ve seçimler değiştiğinde tetikle
setTimeout(centerSelectedRadios, 300);
setTimeout(centerSelectedRadios, 800);
</script>
...
# --- TEK SATIR YATAY KAYDIRILABILIR 15 DAKİKALIK ZAMAN SEÇİCİLERİ ---
