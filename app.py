...
with tab_archive:
    ...
    st.caption("Arşivdeki PDF'leri doğrudan telefonunuzun kendi programıyla önizleyebilir veya indirebilirsiniz.")
    if st.session_state.pdf_archive:
        for filename, b64_pdf in list(st.session_state.pdf_archive.items()):
            ...
            with st.expander(f"🔍 Önizle: {filename}"):
                try:
                    base64_pdf_str = f"data:application/pdf;base64,{b64_pdf}"
                    st.markdown(f'<iframe src="{base64_pdf_str}" width="100%" height="500px" type="application/pdf"></iframe>', unsafe_allow_html=True)
                except Exception:
                    st.error("Önizleme yüklenemedi.")
    else:
        st.info("Arşiv boş.")
...
with tab_monthly:
    st.header("📅 Aylık Rapor ve Çalışan Kazanç Analizi")
...
with tab_entry:
    ...
    st.markdown("---")
    st.markdown("### 📄 Rapor Oluşturma ve PDF İndirme & Önizleme")
    ...
    if active_projects:
        for proj in active_projects:
            ...
            with col_dl2:
                preview_toggle_key = f"prev_state_{proj}_{week_no}"
                if preview_toggle_key not in st.session_state:
                    st.session_state[preview_toggle_key] = False
                
                if st.button("🔍 Önizle", key=f"btn_prev_{proj}_{week_no}"):
                    st.session_state[preview_toggle_key] = not st.session_state[preview_toggle_key]
            with col_dl3:
                if current_week_key in st.session_state.completed_weeks:
            ...
            # Önizleme açık ise iframe ile göster
            if st.session_state.get(f"prev_state_{proj}_{week_no}", False):
                st.markdown(f'<iframe src="data:application/pdf;base64,{pdf_b64}" width="100%" height="500px" type="application/pdf"></iframe>', unsafe_allow_html=True)
    else:
