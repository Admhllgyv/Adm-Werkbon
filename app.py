...
# Session State & DB Senkronizasyonu (Doğru Firma Bilgileriyle)
...
def get_logo_io():
    if st.session_state.comp_logo_b64:
        try:
            ...
            # Şeffaf PNG'ler için beyaz arka plan desteği
...
with tab_settings:
    ...
    uploaded_logo = st.file_uploader("Firma Logosu Yükle veya Değiştir (PNG / JPG)", type=["jpg", "jpeg", "png"])
    ...
    all_weeks_entries = []
    all_weeks_expenses = []
    for wk_key, wk_data in st.session_state.weekly_data.items():
        for entry in wk_data.get("entries", []):
            e_c = entry.copy()
            e_c["Hafta"] = wk_key
            all_weeks_entries.append(e_c)
        for exp in wk_data.get("expenses", []):
            ex_c = exp.copy()
            ex_c["Hafta"] = wk_key
            all_weeks_expenses.append(ex_c)

    try:
        excel_buffer = io.BytesIO()
        with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
            if all_weeks_entries:
                pd.DataFrame(all_weeks_entries).to_excel(writer, sheet_name='Calisma_Saatleri', index=False)
            else:
                pd.DataFrame([{"Bilgi": "Henüz kayıt yok"}]).to_excel(writer, sheet_name='Calisma_Saatleri', index=False)
            if all_weeks_expenses:
                pd.DataFrame(all_weeks_expenses).to_excel(writer, sheet_name='Giderler', index=False)
            else:
                pd.DataFrame([{"Bilgi": "Henüz gider yok"}]).to_excel(writer, sheet_name='Giderler', index=False)
        excel_buffer.seek(0)
        st.download_button(
            label="📊 Tek Tıkla Excel Raporunu İndir (.xlsx)",
            data=excel_buffer,
            file_name=f"adm_technics_tum_veriler_{datetime.now().strftime('%Y%m%d')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    except Exception as e:
        st.info("Excel raporu hazırlanırken CSV formatı alternatif olarak kullanılabilir:")
        if all_weeks_entries:
            csv_data = pd.DataFrame(all_weeks_entries).to_csv(index=False).encode('utf-8')
            st.download_button("📊 CSV Raporu İndir", data=csv_data, file_name="adm_technics_rapor.csv", mime="text/csv")
...
with tab_projects:
    st.header("📁 Proje Yönetimi ve Detaylı Çalışma Geçmişi")
    st.markdown("Buradan projelerinizi güncelleyebilir ve hangi projede kimin ne kadar çalıştığını inceleyebilirsiniz.")
    ...
    st.subheader("Mevcut Projeler (Düzenle / Sil / Geçmişi Gör)")
    ...
    for idx, prj in enumerate(sorted_projects):
        with st.expander(f"📌 {prj['code']} - {prj['name']}"):
            ...
            st.markdown("---")
            st.markdown(f"📊 **{prj['name']} ({prj['code']}) - Çalışma Geçmişi ve Detaylar**")
            
            proj_full_label = f"{prj['code']} - {prj['name']}"
            matched_entries = []
            matched_expenses = []
            
            for wk_key, wk_data in st.session_state.weekly_data.items():
                for entry in wk_data.get("entries", []):
                    if entry.get("Proje") == proj_full_label or entry.get("Proje") == prj['name'] or prj['code'] in entry.get("Proje", ""):
                        e_copy = entry.copy()
                        e_copy["Hafta"] = wk_key
                        matched_entries.append(e_copy)
                for exp in wk_data.get("expenses", []):
                    if exp.get("Proje") == proj_full_label or exp.get("Proje") == prj['name'] or prj['code'] in exp.get("Proje", ""):
                        ex_copy = exp.copy()
                        ex_copy["Hafta"] = wk_key
                        matched_expenses.append(ex_copy)
            
            if matched_entries:
                df_m = pd.DataFrame(matched_entries)
                total_proj_hours = df_m["Saat"].sum()
                st.caption(f"Toplam Çalışma Saati: **{total_proj_hours} saat**")
                html_table = df_m[["Hafta", "Gün", "Tarih", "Çalışan", "Başlangıç", "Mola", "Bitiş", "Saat"]].to_html(index=False, classes='resp-table')
                st.markdown(html_table, unsafe_allow_html=True)
            else:
                st.info("Bu projeye ait henüz kayıtlı çalışma saati bulunmuyor.")

            if matched_expenses:
                st.markdown("---")
                st.caption("🏷️ Bu Projeye Ait Giderler / Fişler")
                df_me = pd.DataFrame(matched_expenses)
                html_exp_table = df_me[["Hafta", "Gün", "Tarih", "Kod", "Açıklama", "Tutar"]].to_html(index=False, classes='resp-table')
                st.markdown(html_exp_table, unsafe_allow_html=True)

with tab_archive:
    st.header("📚 PDF Arşivi (İndirilenler Merkezi)")
    st.markdown("Oluşturduğunuz ve indirdiğiniz tüm PDF raporları burada kalıcı olarak saklanır.")
    ...
    if st.session_state.pdf_archive:
        ...
        st.info("Henüz arşivde PDF bulunmuyor. Haftalık rapor oluşturduğunuzda buraya eklenecektir.")
...
with tab_preview:
    ...
    if p_entries:
        ...
        st.markdown(worker_totals.to_html(index=False, classes='resp-table'), unsafe_allow_html=True)
        
        st.subheader("📌 Proje Bazlı Toplam Saatler")
        project_totals = df_p.groupby("Proje")["Saat"].sum().reset_index()
        st.markdown(project_totals.to_html(index=False, classes='resp-table'), unsafe_allow_html=True)
        
        st.subheader("📋 Detaylı Çalışma Listesi")
        st.markdown(df_p[["Gün", "Tarih", "Proje", "Çalışan", "Başlangıç", "Mola", "Bitiş", "Saat"]].to_html(index=False, classes='resp-table'), unsafe_allow_html=True)
    else:
    ...
    if p_expenses:
        ...
        st.markdown(df_exp[["Gün", "Tarih", "Proje", "Kod", "Açıklama", "Tutar"]].to_html(index=False, classes='resp-table'), unsafe_allow_html=True)
...
with tab_entry:
    ...
    with col_wk:
        ...
        for idx, w in enumerate(week_options):
            w_key_opt = f"{sel_year}-W{w}"
            if w_key_opt in st.session_state.completed_weeks:
    ...
    def update_worker_defaults(day_name, row_idx, wk_no):
        w_k = f"w_{day_name}_{row_idx}_{wk_no}"
        s_k = f"s_choice_{day_name}_{row_idx}_{wk_no}"
        p_k = f"p_choice_{day_name}_{row_idx}_{wk_no}"
        e_k = f"e_choice_{day_name}_{row_idx}_{wk_no}"
        
        selected_worker = st.session_state.get(w_k)
        if selected_worker == "Adem":
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                w_key = f"w_{day}_{i}_{week_no}"
                s_choice_key = f"s_choice_{day}_{i}_{week_no}"
                p_choice_key = f"p_choice_{day}_{i}_{week_no}"
                e_choice_key = f"e_choice_{day}_{i}_{week_no}"
                
                if w_key not in st.session_state:
                    st.session_state[w_key] = saved_e.get("Çalışan", "")
                
                initial_worker = st.session_state[w_key]
                
                if s_choice_key not in st.session_state:
                    if "Başlangıç" in saved_e:
                        st.session_state[s_choice_key] = saved_e["Başlangıç"]
                    else:
                        st.session_state[s_choice_key] = "05:30" if initial_worker == "Adem" else "07:00"
                        
                if p_choice_key not in st.session_state:
                    if "Mola" in saved_e:
                        st.session_state[p_choice_key] = saved_e["Mola"]
                    else:
                        st.session_state[p_choice_key] = "01:45" if initial_worker == "Adem" else "00:45"
                        
                if e_choice_key not in st.session_state:
                    if "Bitiş" in saved_e:
                        st.session_state[e_choice_key] = saved_e["Bitiş"]
                    else:
                        st.session_state[e_choice_key] = "17:15" if initial_worker == "Adem" else "15:45"

                # --- YATAY PROFESYONEL DÜZEN (YAN YANA SÜTUNLAR) ---
                col_row_main = st.columns([1.5, 2.8, 2.8, 2.8])
                with col_row_main[0]:
                    wrk = st.selectbox(f"Çalışan {i+1}", w_list, key=w_key, on_change=update_worker_defaults, args=(day, i, week_no))
                with col_row_main[1]:
                    s_choice = st.selectbox(f"Başlangıç {i+1}", start_slots, key=s_choice_key)
                    start = st.text_input(f"Manuel Başl. {i+1}", value=st.session_state.get(f"s_man_{day}_{i}_{week_no}", s_choice if s_choice != "Manuel Giriş" else "07:00"), key=f"s_man_{day}_{i}_{week_no}") if s_choice == "Manuel Giriş" else s_choice
                with col_row_main[2]:
                    p_choice = st.selectbox(f"Mola {i+1}", pause_slots, key=p_choice_key)
                    pause = st.text_input(f"Manuel Mola {i+1}", value=st.session_state.get(f"p_man_{day}_{i}_{week_no}", p_choice if p_choice != "Manuel Giriş" else "00:45"), key=f"p_man_{day}_{i}_{week_no}") if p_choice == "Manuel Giriş" else p_choice
                with col_row_main[3]:
                    e_choice = st.selectbox(f"Bitiş {i+1}", end_slots, key=e_choice_key)
                    stop = st.text_input(f"Manuel Bitiş {i+1}", value=st.session_state.get(f"e_man_{day}_{i}_{week_no}", e_choice if e_choice != "Manuel Giriş" else "15:45"), key=f"e_man_{day}_{i}_{week_no}") if e_choice == "Manuel Giriş" else e_choice
                
                calculated_hours = calculate_net_hours(start, pause, stop)
                st.caption(f"⏱️ {wrk if wrk else 'Çalışan'} - Net: **{calculated_hours} saat** ({start} - {stop}, Mola: {pause})")
                st.markdown("---")
    ...
    def create_pdf(project_name, entries, expenses, week, year):
        ...
        # --- Modern & Profesyonel PDF Stil Tanımlamaları ---
        ...
        header_text = f"<b><font size=11 color='#0F172A'>{st.session_state.comp_name}</font></b><br/><font size=7 color='#475569'>{st.session_state.comp_address}<br/>BTW: {st.session_state.comp_btw}</font>"
        ...
        header_data = [
            [
                header_left_elements,
                Paragraph(f"<b>Weeknummer:</b> {week} ({year})<br/><b>Project / Klant:</b> {project_name}", meta_p_style)
            ]
        ]
        ...
        # --- Çalışan Bazlı Düzenli Sıralama ---
        ...
        for e in sorted_entries:
            ...
            total_hours += e["Saat"]
            w_name = e["Çalışan"]
            worker_hours[w_name] = worker_hours.get(w_name, 0.0) + e["Saat"]
        ...
        if receipts_with_images:
            ...
            story.append(Paragraph("<b>Bonnetjes en Facturen (Documenten)</b>", ParagraphStyle('ReceiptTitle', fontName='Helvetica-Bold', fontSize=11, textColor=primary_color)))
            ...
            for idx, ex in enumerate(receipts_with_images):
                try:
                    ...
                    draw.rectangle([(0, 0), (w_img, banner_height)], fill=(15, 23, 42))
            ...
            if img_table_data:
                t_receipts = Table(img_table_data, colWidths=[270, 275])
    ...
    if all_entries or all_expenses:
        if st.button("🚀 Bu Haftanın PDF Raporlarını Oluştur", type="primary", key=f"btn_create_pdfs_{week_no}"):
            ...
            st.success("Haftalık PDF raporları başarıyla oluşturuldu ve arşive eklendi!")
            st.rerun()
            
        # PDF'ler her zaman arşivle ve veritabanı ile senkronize kalır (kaybolmaz)
        ...
        if active_projects:
            ...
            for proj in active_projects:
                ...
                # Her üretimde arşiv güncellenir ve üzerine yazılır
        ...
        st.info("Bu hafta için henüz çalışma kaydı veya gider fişi girilmedi. Veri girdiğinizde PDF indirme butonları otomatik olarak aktifleşecektir.")
