        ...
        with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
            ...
            for wk_k, wk_val in st.session_state.weekly_data.items():
                for exp in wk_val.get("expenses", []):
                    # Excel'e base64 görsel verisini yazmamak için kopyalayıp çıkaralım
                    exp_copy = exp.copy()
...
with tab_entry:
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            num_projects = st.number_input(f"Günlük Proje / Görev Sayısı ({day})", min_value=0, max_value=5, value=0, key=f"num_proj_{day}_{week_no}")
            ...
            for p_i in range(int(num_projects)):
                ...
                if p_key not in st.session_state:
                    st.session_state[p_key] = "Seçiniz"
                ...
                if proj_sel != "Seçiniz":
                    ...
                    num_workers = st.number_input(f"Çalışan Sayısı ({proj_sel} - {day})", min_value=0, max_value=5, value=0, key=f"num_w_{day}_{p_i}_{week_no}")
                    ...
                    for w_i in range(int(num_workers)):
                        ...
                        if w_key not in st.session_state:
                            st.session_state[w_key] = "Seçiniz"
                        ...
                        if wrk != "Seçiniz":
                            ...
                            if man_s:
                                final_start = st.text_input("Manuel Başlangıç", value=st.session_state.get(s_key, def_s), key=f"s_man_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed")
                            else:
                            ...
                            if man_p:
                                final_pause = st.text_input("Manuel Mola", value=st.session_state.get(pk_key, def_p), key=f"p_man_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed")
                            else:
                            ...
                            if man_e:
                                final_stop = st.text_input("Manuel Bitiş", value=st.session_state.get(e_key, def_e), key=f"e_man_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed")
                            else:
    ...
    def create_pdf(project_name, entries, expenses, week, year):
        ...
        # --- FİŞ / GİDER GÖRSELLERİNİ PDF'E EKLEME ---
        has_images = any(ex.get("Görsel_b64") for ex in proj_expenses)
        if has_images:
            story.append(PageBreak())
            story.append(Paragraph("<b>Bijlagen / Bonnetjes (Fiş ve Gider Görselleri):</b>", ParagraphStyle('ImgHeader', fontName='Helvetica-Bold', fontSize=10, textColor=primary_color)))
            ...
            for ex in proj_expenses:
                if ex.get("Görsel_b64"):
                    try:
                        ...
                        story.append(Paragraph(f"<b>{ex['Kod']}</b> - {ex['Açıklama']} (€{ex['Tutar']:.2f}) - {day_to_nl.get(ex['Gün'], ex['Gün'])}", bold_sub_style))
