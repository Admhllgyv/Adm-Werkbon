                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                with col_t1:
                                    final_start = st.text_input("Başlangıç (HH:MM)", value=st.session_state.get(s_key, def_s), key=f"s_{day}{p_i}{w_i}_{week_no}", disabled=is_week_locked)
                                    st.session_state[s_key] = final_start
                                with col_t2:
                                    final_pause = st.text_input("Mola (HH:MM)", value=st.session_state.get(pk_key, def_p), key=f"p_{day}{p_i}{w_i}_{week_no}", disabled=is_week_locked)
                                    st.session_state[pk_key] = final_pause
                                with col_t3:
                                    final_stop = st.text_input("Bitiş (HH:MM)", value=st.session_state.get(e_key, def_e), key=f"e_{day}{p_i}{w_i}_{week_no}", disabled=is_week_locked)
