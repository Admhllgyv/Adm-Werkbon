...
st.markdown("""
    ...
    /* Tabloların ekrana tam oturması ve taşmaması için */
    .resp-table {
        width: 100% !important;
...
pause_slots = ["00:15", "00:30", "00:45", "01:00", "01:15", "01:30", "01:45", "02:00", "Manuel"]
...
with tab_settings:
    ...
    st.markdown("---")
    st.subheader("Gider / Malzeme Kategorileri (Parking, Boru, Kablo vb.)")
    ...
    for idx, prj in enumerate(st.session_state.projects):
        with st.expander(f"📌 {prj['code']} - {prj['name']}"):
            ...
            if matched_entries:
                ...
                st.markdown(df_proj[["Hafta", "Gün", "Tarih", "Çalışan", "Başlangıç", "Mola", "Bitiş", "Saat"]].to_html(index=False, classes='resp-table'), unsafe_allow_html=True)
            else:
...
with tab_archive:
    ...
    if st.session_state.pdf_archive:
        for filename, b64_pdf in list(st.session_state.pdf_archive.items()):
            ...
            with col_ar2:
                try:
                    st.download_button("📥 İndir", data=base64.b64decode(b64_pdf), file_name=filename, mime="application/pdf", key=f"arch_{filename}")
                except Exception:
                    pass
    else:
        st.info("Arşiv boş.")
...
with tab_preview:
    ...
    with col_pyr:
        prev_sel_year = st.selectbox("Yıl", list(range(2026, 2036)), index=0, key="prev_y")
    with col_pwk:
        prev_week_no = st.selectbox("Hafta No", list(range(1, 54)), index=current_iso_week-1, key="prev_w")
...
with tab_entry:
    ...
    with col_wk:
        ...
        formatted_opts = []
        def_idx = current_iso_week - 1 if current_iso_week <= 53 else 0
        for w in week_options:
            k = f"{sel_year}-W{w}"
            lbl = f"🔴 Week {w} [Tamamlandı]" if k in st.session_state.completed_weeks else f"Week {w}"
            formatted_opts.append((w, lbl))
            if w == current_iso_week and sel_year == current_iso_year:
                def_idx = w - 1
                
        sel_tuple = st.selectbox("Hafta Seçimi", options=formatted_opts, format_func=lambda x: x[1], index=def_idx)
        week_no = sel_tuple[0]
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                s_key = f"s_choice_{day}_{i}_{week_no}"
                p_key = f"p_choice_{day}_{i}_{week_no}"
                e_key = f"e_choice_{day}_{i}_{week_no}"
                ...
                if s_key not in st.session_state:
                    st.session_state[s_key] = saved_e.get("Başlangıç", "05:30" if init_w == "Adem" else "07:00")
                if p_key not in st.session_state:
                    st.session_state[p_key] = saved_e.get("Mola", "01:45" if init_w == "Adem" else "00:45")
                if e_key not in st.session_state:
                    st.session_state[e_key] = saved_e.get("Bitiş", "17:15" if init_w == "Adem" else "15:45")

                # --- ÇALIŞAN ÜSTTE, SAATLER (BAŞLANGIÇ, MOLA, BİTİŞ) 3 SÜTUN YAN YANA VE KOMPAKT ---
                wrk = st.selectbox(f"Çalışan {i+1} ({day})", w_list, key=w_key, on_change=update_worker_defaults, args=(day, i, week_no))
                
                st.caption("Başlangıç | Mola | Bitiş")
                time_cols = st.columns(3)
                with time_cols[0]:
                    s_choice = st.selectbox(f"Baş {i+1}", start_slots, key=s_key, label_visibility="collapsed")
                    start = st.text_input(f"M.Baş {i+1}", value=saved_e.get("Başlangıç", "07:00"), key=f"s_man_{day}_{i}_{week_no}", label_visibility="collapsed") if s_choice == "Manuel" else s_choice
                with time_cols[1]:
                    p_choice = st.selectbox(f"Mol {i+1}", pause_slots, key=p_key, label_visibility="collapsed")
                    pause = st.text_input(f"M.Mol {i+1}", value=saved_e.get("Mola", "00:45"), key=f"p_man_{day}_{i}_{week_no}", label_visibility="collapsed") if p_choice == "Manuel" else p_choice
                with time_cols[2]:
                    e_choice = st.selectbox(f"Bit {i+1}", end_slots, key=e_key, label_visibility="collapsed")
                    stop = st.text_input(f"M.Bit {i+1}", value=saved_e.get("Bitiş", "15:45"), key=f"e_man_{day}_{i}_{week_no}", label_visibility="collapsed") if e_choice == "Manuel" else e_choice
                
                calc_hours = calculate_net_hours(start, pause, stop)
                st.caption(f"⏱️ **{wrk or 'Seçilmedi'}** | Net: **{calc_hours} saat** ({start}-{stop}, Mola: {pause})")
                st.markdown("---")
