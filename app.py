...
# --- YENİ KLAVYESİZ & KAYDIRMASIZ AKILLI SAAT SEÇİCİ ---
def time_picker_component(label, min_h, max_h, session_key, default_val, disabled=False):
    st.markdown(f"<span style='font-size:12px; font-weight:600; color:#1F4E78;'>{label}</span>", unsafe_allow_html=True)
    
    try:
        parts = default_val.split(":")
        def_h = parts[0]
        def_m = parts[1] if len(parts) > 1 else "00"
    except Exception:
        def_h = f"{min_h:02d}"
        def_m = "00"
        
    man_key = f"man_{session_key}"
    h_key = f"h_{session_key}"
    m_key = f"m_{session_key}"
    
    if man_key not in st.session_state:
        st.session_state[man_key] = False
        
    is_manual = st.checkbox(f"Manuel Giriş ({label})", key=f"chk_{man_key}", disabled=disabled)
    
    if is_manual:
        return st.text_input(f"Manuel {label}", value=default_val, key=f"txt_{session_key}", disabled=disabled, label_visibility="collapsed")
    else:
        hours = [f"{h:02d}" for h in range(min_h, max_h + 1)]
        minutes = ["00", "15", "30", "45"]
        
        if h_key not in st.session_state or st.session_state[h_key] not in hours:
            st.session_state[h_key] = def_h if def_h in hours else hours[0]
        if m_key not in st.session_state or st.session_state[m_key] not in minutes:
            st.session_state[m_key] = def_m if def_m in minutes else minutes[0]
            
        col_h, col_m = st.columns([3, 2])
        with col_h:
            st.markdown("<span style='font-size:11px; color:#475569; font-weight:bold;'>Saat:</span>", unsafe_allow_html=True)
            sel_h = st.radio(f"{label} Saat", hours, horizontal=True, key=h_key, disabled=disabled, label_visibility="collapsed")
        with col_m:
            st.markdown("<span style='font-size:11px; color:#475569; font-weight:bold;'>Dakika:</span>", unsafe_allow_html=True)
            sel_m = st.radio(f"{label} Dakika", minutes, horizontal=True, key=m_key, disabled=disabled, label_visibility="collapsed")
            
        return f"{sel_h}:{sel_m}"

def break_picker_component(label, session_key, default_val, disabled=False):
    st.markdown(f"<span style='font-size:12px; font-weight:600; color:#1F4E78;'>{label}</span>", unsafe_allow_html=True)
    
    break_options = ["00:00", "00:15", "00:30", "00:45", "01:00", "01:15", "01:30", "01:45", "02:00"]
    man_key = f"man_break_{session_key}"
    b_key = f"b_{session_key}"
    
    is_manual = st.checkbox(f"Manuel Mola ({label})", key=f"chk_{man_key}", disabled=disabled)
    
    if is_manual:
        return st.text_input(f"Manuel {label}", value=default_val, key=f"txt_break_{session_key}", disabled=disabled, label_visibility="collapsed")
    else:
        if b_key not in st.session_state or st.session_state[b_key] not in break_options:
            st.session_state[b_key] = default_val if default_val in break_options else "00:45"
            
        sel_break = st.radio(label, break_options, horizontal=True, key=b_key, disabled=disabled, label_visibility="collapsed")
        return sel_break
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                # Yeni Saat Seçiciler (Başlangıç: 03-12, Mola, Bitiş: 11-20 + Manuel)
                                final_start = time_picker_component("Başlangıç Saati (03:00 - 12:00)", 3, 12, f"s_{day}{p_i}{w_i}_{week_no}", st.session_state.get(s_key, def_s), disabled=is_week_locked)
                                st.session_state[s_key] = final_start

                                final_pause = break_picker_component("Mola Süresi", f"p_{day}{p_i}{w_i}_{week_no}", st.session_state.get(pk_key, def_p), disabled=is_week_locked)
                                st.session_state[pk_key] = final_pause

                                final_stop = time_picker_component("Bitiş Saati (11:00 - 20:00)", 11, 20, f"e_{day}{p_i}{w_i}_{week_no}", st.session_state.get(e_key, def_e), disabled=is_week_locked)
