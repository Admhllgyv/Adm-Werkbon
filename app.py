...
st.markdown("""
    <style>
    /* Seçilen metinlerin görünmesini sağlarken, sadece açılır menü arama çubuğunun klavye tetiklemesini engelle */
    div[data-baseweb="popover"] input[aria-autocomplete="list"],
...
# Geniş ve çok seçenekli saat slotları
...
with tab_entry:
    ...
    with col_wk:
        week_no = st.selectbox("Hafta Seçimi", list(range(1, 54)), index=current_iso_week-1)
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            # Çalışan Sayısı sekmesinin hemen altına ve ilk çalışanın üstüne gri ayırıcı çizgi
            if num_rows > 0:
                st.markdown("<hr style='border: 1px solid #CBD5E1; margin: 12px 0;'>", unsafe_allow_html=True)

            for i in range(int(num_rows)):
                # Çalışanlar arasına gri ayırıcı çizgi
                if i > 0:
                    st.markdown("<hr style='border: 1px solid #CBD5E1; margin: 12px 0;'>", unsafe_allow_html=True)
