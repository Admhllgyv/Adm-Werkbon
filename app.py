...
st.markdown("""
    <style>
    /* HAP (PILLS) SEÇENEKLERİNİ KESİNLİKLE TEK SATIRDA (ŞERİT) TUTAN KOD */
    div[data-testid="stPills"] > div {
        display: flex !important;
        flex-direction: row !important;
        flex-wrap: nowrap !important; /* Alt satıra inmeyi (kalabalığı) KESİNLİKLE yasakla */
        overflow-x: auto !important;  /* Sadece yatayda kaydırmaya izin ver */
        overflow-y: hidden !important;
        -webkit-overflow-scrolling: touch !important; /* Mobilde parmakla akıcı kaydırma (swipe) */
        scrollbar-width: none !important; /* Firefox için scrollbar'ı gizle */
        gap: 8px !important; /* Butonlar arası boşluk */
        padding-bottom: 2px !important;
    }
    
    /* Chrome, Safari ve mobil tarayıcılar için çirkin scrollbar çubuğunu gizle */
    div[data-testid="stPills"] > div::-webkit-scrollbar {
        display: none !important;
    }

    /* Butonların içindeki yazıların ezilmesini veya satır atlamasını önle */
    div[data-testid="stPills"] label {
        white-space: nowrap !important; /* Yazıyı tek satırda tut */
        flex-shrink: 0 !important; /* Ekrana sığmak için butonların daralmasını engelle */
...
with tab_preview:
    ...
    # Yıl ve Hafta Tek Satır Şerit
    prev_sel_year_str = st.pills("Yıl (Kaydırın ↔️)", years_list, key="prev_y_pill")
    prev_sel_year = int(prev_sel_year_str) if prev_sel_year_str else current_iso_year
    
    prev_week_no_str = st.pills("Hafta (Kaydırın ↔️)", weeks_list, key="prev_w_pill")
...
with tab_entry:
    ...
    # Yıl ve Hafta Tek Satır Şerit (Veri Girişi)
