...
<script>
// Seçili saati yatay şeridin tam merkezine sabitleyen kesin çözüm motoru
function forceCenterActiveRadio() {
    const radioContainers = document.querySelectorAll('div[data-testid="stRadio"] > div');
    radioContainers.forEach(container => {
        const checkedInput = container.querySelector('input[type="radio"]:checked');
        if (checkedInput) {
            const label = checkedInput.closest('label');
            if (label) {
                // Matematiksel tam ortalama hesabı
                const containerWidth = container.clientWidth;
                const labelLeft = label.offsetLeft;
                const labelWidth = label.clientWidth;
                const targetScrollLeft = labelLeft - (containerWidth / 2) + (labelWidth / 2);
                
                container.scrollTo({
                    left: targetScrollLeft,
                    behavior: 'auto' // Anında kilitlensin, gecikme olmasın
                });
            }
        }
    });
...
// Sayfa yüklendiğinde kademeli tetikle
window.addEventListener('DOMContentLoaded', () => {
    [50, 200, 500, 1000].forEach(t => setTimeout(forceCenterActiveRadio, t));
});

// Streamlit her rerun olduğunda yakalamak için MutationObserver
const observer = new MutationObserver((mutations) => {
    forceCenterActiveRadio();
});
...
// Ekstra güvenlik için her tıklama ve değişimde çalıştır
document.addEventListener('change', (e) => {
    setTimeout(forceCenterActiveRadio, 50);
});
setTimeout(forceCenterActiveRadio, 300);
</script>
...
# --- BİRLEŞTİRİLMİŞ 15'ER DAKİKALIK YATAY ÇİP SEÇİCİLERİ ---
def time_picker_component_unified(label, min_h, max_h, session_key, default_val, disabled=False):
    st.markdown(f"<span style='font-size:11px; font-weight:bold; color:#1F4E78;'>{label}</span>", unsafe_allow_html=True)
    ...
    for h in range(min_h, max_h + 1):
        for m in ["00", "15", "30", "45"]:
            if h == max_h and m != "00":
                break
            time_options.append(f"{h:02d}:{m}")
            
    man_key = f"man_{session_key}"
    t_key = f"t_{session_key}"
    ...
    with col_chk:
        is_manual = st.checkbox("Manuel", key=f"chk_{man_key}", disabled=disabled)
    ...
    if is_manual:
        ...
        if t_key not in st.session_state or st.session_state[t_key] not in time_options:
            st.session_state[t_key] = default_val if default_val in time_options else time_options[0]
            
        sel_time = st.radio(label, time_options, horizontal=True, key=t_key, disabled=disabled, label_visibility="collapsed")
        return sel_time
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                final_start = time_picker_component_unified("Başlangıç (03:00 - 12:00)", 3, 12, f"s_{day}{p_i}{w_i}_{week_no}", st.session_state.get(s_key, def_s), disabled=is_week_locked)
                                st.session_state[s_key] = final_start

                                final_pause = break_picker_component("Mola", f"p_{day}{p_i}{w_i}_{week_no}", st.session_state.get(pk_key, def_p), disabled=is_week_locked)
                                st.session_state[pk_key] = final_pause

                                final_stop = time_picker_component_unified("Bitiş (11:00 - 20:00)", 11, 20, f"e_{day}{p_i}{w_i}_{week_no}", st.session_state.get(e_key, def_e), disabled=is_week_locked)
