...
# Samsung S23 Ultra & Mobile Optimized Compact CSS (Parmak Payı ve Font Optimizasyonu)
st.markdown("""
    <style>
    .block-container {
        padding-left: 0.8rem;
...
# Projeler Listesi (Proje Adı ve Proje Numarası/Kodu)
if 'projects' not in st.session_state:
    st.session_state.projects = [
        {"name": "Kruidvat Diepenbeek", "code": "PRJ-2026-01"},
        {"name": "ICI PARIS XL Hasselt", "code": "PRJ-2026-02"}
    ]

# Gider Kategorileri (Ayarlardan yönetilebilir)
if 'expense_categories' not in st.session_state:
    st.session_state.expense_categories = ["Malzeme", "Su borusu", "Şalter", "Parking", "Kabel", "Verlichting"]

# Haftalık verileri ve tamamlanan (kırmızı olacak) haftaları saklamak için
...
# Tabs for Navigation: Eklenen Proje Yönetimi Sekmesi ile birlikte
...
with tab_settings:
    ...
    st.markdown("---")
    st.subheader("Gider / Malzeme Açıklamaları (Açılır Liste Ayarı)")
    col_e1, col_e2 = st.columns([2, 1])
    with col_e1:
        new_exp_desc = st.text_input("Yeni Gider Tanımı (Örn: Şalter, Parking vb.)")
    with col_e2:
        st.write("")
        st.write("")
...
with tab_projects:
    ...
    with st.form("add_project_form"):
        ...
        p_code = st.text_input("Proje Numarası / Kodu (Örn: PRJ-2026-03)")
        submitted = st.form_submit_button("Yeni Proje Kaydet")
        if submitted and p_name:
            st.session_state.projects.append({"name": p_name, "code": p_code if p_code else "PRJ-GENEL"})
            st.success(f"'{p_name}' başarıyla eklendi!")
            st.rerun()
            
    st.subheader("Mevcut Projeler ve İlgili Dosyalar")
    for prj in st.session_state.projects:
        with st.expander(f"📌 {prj['name']} ({prj['code']})"):
...
with tab_entry:
    # Başlık ve Hafta / Geçmiş Hafta Seçimi (Kırmızı tamamlandı göstergesi ile)
    ...
    with col_wk:
        ...
        # Tamamlanan haftaları kırmızı renk veya işaretle belirtme
    ...
    for day in days:
        with st.expander(f"📌 {day} ({day_to_nl[day]})", expanded=(day == "Pazartesi")):
            ...
            for i in range(int(num_rows)):
                ...
                with cols[0]:
                    proj = st.selectbox("Proje Seç", project_names_list, key=f"p_{day}_{i}_{week_no}")
                with cols[1]:
            ...
            for j in range(int(num_exp)):
                ...
                if exp_amount > 0:
                    ...
                    all_expenses.append({
                        "Gün": day,
                        "Proje": proj if 'proj' in locals() else project_names_list[0],
                        "Kod": doc_code,
                        "Açıklama": exp_desc,
                        "Tutar": exp_amount,
                        "Görsel": exp_file
    ...
    # Verileri session state'e bu hafta için kaydedelim
...
with tab_preview:
    ...
    # 1. İstek: En üstte kişilerin toplam haftalık ne kadar çalıştığı net görünsün
    if all_entries:
...
# PDF Generation Function (Tamamen Hollandaca ve 3-4 görsel sığdırılabilen kompakt ek düzeni)
def create_pdf(project_name, entries, expenses, week, year):
    ...
    # Tamamen Hollandaca Başlıklar ve Gün İsimleri
    ...
    # 6. İstek: Görseller tek sayfaya yığılmasın, bir sayfaya 3-4 tane (2 sütunlu tablo halinde) sığacak şekilde kompakt yerleştirilsin
    ...
    if receipts_with_images:
        ...
        for idx, ex in enumerate(receipts_with_images):
            try:
                ...
                # Tek sayfaya 3-4 tane sığması için kompakt boyut (genişlik 220, yükseklik orantılı max 200)
                aspect = h_img / w_img
                img_w = 220
                img_h = img_w * aspect
                if img_h > 210:
                ...
                # 2 sütunlu yapı oluşturmak için her 2 görselde bir satırı kapat
                if len(row_cells) == 2:
        ...
        # Kalan tekli hücre varsa ekle
        if row_cells:
            row_cells.append("") # boş hücre
            img_table_data.append(row_cells)
...
# İndir Butonu ve Hafta Tamamlama İşaretleme
st.markdown("---")
    ...
    for proj in unique_projects:
        ...
        with col_dl1:
            btn_clicked = st.download_button(
                ...
                data=pdf_file,
                file_name=f"Werkbon_Week{week_no}_{sel_year}_{proj.replace(' ', '_')}.pdf",
        ...
        with col_dl2:
            if st.button(f"✅ Tamamlandı İşaretle", key=f"mark_{proj}_{week_no}"):
                ...
                st.success("Hafta kırmızı/tamamlandı işaretlendi!")
                st.rerun()
else:
