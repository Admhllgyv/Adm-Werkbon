...
/* SAAT VE DAKİKA KUTULARININ TELEFonda BİLE YAN YANA KALMASINI SAĞLAYAN KRİTİK CSS */
...
/* Selectbox kutularını kompakt ve şık yap */
...
# --- PROFESYONEL VE KOMPAKT YAN YANA AÇILIR MENÜ SAAT SEÇİCİ ---
def professional_time_picker(label, session_key, default_val="08:00", disabled=False):
    ...
    h_key = f"h_sel_{session_key}"
    m_key = f"m_sel_{session_key}"
    ...
    with col_h:
        current_h = st.session_state[h_key] if st.session_state[h_key] in hours else def_h
        selected_h = st.selectbox(
            "Saat",
            hours,
            index=hours.index(current_h),
            format_func=lambda x: f"{x:02d} Saat",
            key=f"sb_h_{session_key}",
            disabled=disabled,
            label_visibility="collapsed"
        )
        st.session_state[h_key] = selected_h
        
    with col_m:
        current_m = st.session_state[m_key] if st.session_state[m_key] in minutes else def_m
        selected_m = st.selectbox(
            "Dakika",
            minutes,
            index=minutes.index(current_m),
            format_func=lambda x: f"{x:02d} Dakika",
            key=f"sb_m_{session_key}",
            disabled=disabled,
            label_visibility="collapsed"
        )
        st.session_state[m_key] = selected_m
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                # Kompakt Yan Yana Açılır Menü Seçiciler
                                final_start = professional_time_picker("Başlangıç Saati", f"start_{day}{p_i}{w_i}_{week_no}", st.session_state.get(s_key, def_s), disabled=is_week_locked)
                                st.session_state[s_key] = final_start

                                final_pause = professional_time_picker("Mola Süresi", f"pause_{day}{p_i}{w_i}_{week_no}", st.session_state.get(pk_key, def_p), disabled=is_week_locked)
                                st.session_state[pk_key] = final_pause

                                final_stop = professional_time_picker("Bitiş Saati", f"stop_{day}{p_i}{w_i}_{week_no}", st.session_state.get(e_key, def_e), disabled=is_week_locked)
