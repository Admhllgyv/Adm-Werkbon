...
# --- SEKMELER (YENİ "Aylık Rapor" Sekmesi Eklendi) ---
...
with tab_monthly:
    ...
    with m_col2:
        ...
        current_m = datetime.today().month if datetime.today().year == sel_m_year else 8
    ...
    # Seçilen aya ait kayıtları haftalık verilerden filtrele
    monthly_entries = []
    monthly_expenses = []
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                with col_s2:
                                    man_s = st.checkbox("Manuel", key=f"man_s_{day}{p_i}{w_i}_{week_no}")

                                if man_s:
                                    final_start = st.text_input("Manuel Başlangıç", value=st.session_state.get(s_key, def_s), key=f"s_man_{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed")
                                else:
                                    curr_s = st.session_state.get(s_key, def_s)
                                    if curr_s not in start_slots:
                                        curr_s = start_slots[0]
                                    final_start = st.select_slider(
                                        f"Başlangıç {w_i+1}", options=start_slots, value=curr_s, key=f"s_sl{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed"
                                    )
                                ...
                                with col_p1:
                                    st.markdown(f"Mola")
                                with col_p2:
                                    man_p = st.checkbox("Manuel", key=f"man_p_{day}{p_i}{w_i}_{week_no}")

                                if man_p:
                                    final_pause = st.text_input("Manuel Mola", value=st.session_state.get(pk_key, def_p), key=f"p_man_{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed")
                                else:
                                    curr_p = st.session_state.get(pk_key, def_p)
                                    if curr_p not in pause_slots:
                                        curr_p = pause_slots[0]
                                    final_pause = st.select_slider(
                                        f"Mola {w_i+1}", options=pause_slots, value=curr_p, key=f"p_sl{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed"
                                    )
                                ...
                                with col_e1:
                                    st.markdown(f"Bitiş")
                                with col_e2:
                                    man_e = st.checkbox("Manuel", key=f"man_e_{day}{p_i}{w_i}_{week_no}")

                                if man_e:
                                    final_stop = st.text_input("Manuel Bitiş", value=st.session_state.get(e_key, def_e), key=f"e_man_{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed")
                                else:
                                    curr_e = st.session_state.get(e_key, def_e)
                                    if curr_e not in end_slots:
                                        curr_e = end_slots[0]
                                    final_stop = st.select_slider(
                                        f"Bitiş {w_i+1}", options=end_slots, value=curr_e, key=f"e_sl{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed"
                                    )
...
st.markdown("---")
st.markdown("### 📄 Rapor Oluşturma ve PDF İndirme")
...
if active_projects:
    for proj in active_projects:
        ...
        pdf_filename = f"Werkbon_Week{week_no}{sel_year}{proj.replace(' ', '_').replace('(', '').replace(')', '')}.pdf"
        ...
        with col_dl2:
            if current_week_key in st.session_state.completed_weeks:
                if st.button("🔄 Aç", key=f"unmark{proj}{week_no}"):
                    st.session_state.completed_weeks.remove(current_week_key)
                    db_save("completed_weeks", list(st.session_state.completed_weeks))
                    st.rerun()
            else:
                if st.button("✅ Tamamla", key=f"mark{proj}_{week_no}"):
