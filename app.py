...
# --- SAAT VE DAKİKA SEÇİCİ (SAAT + DAKİKA SELECTBOX SİSTEMİ) ---
def time_picker_component_unified(label, min_h, max_h, session_key, default_val, disabled=False):
    ...
    if is_manual:
        ...
        if m_key not in st.session_state or st.session_state[m_key] not in minutes:
            str_def_m = f"{def_m:02d}"
            st.session_state[m_key] = str_def_m if str_def_m in minutes else "00"
            
        col_h, col_m = st.columns(2)
        with col_h:
            sel_h = st.selectbox("Saat", hours, format_func=lambda x: f"{x:02d} Saat", key=h_key, disabled=disabled, label_visibility="collapsed")
        with col_m:
            sel_m = st.selectbox("Dakika", minutes, format_func=lambda x: f"{x} Dakika", key=m_key, disabled=disabled, label_visibility="collapsed")
            
        return f"{sel_h:02d}:{sel_m}"
...
def break_picker_component(label, session_key, default_val, disabled=False):
    ...
    if is_manual:
        ...
        sel_break = st.selectbox(label, break_options, key=b_key, disabled=disabled, label_visibility="collapsed")
        return sel_break
    ...
    def create_pdf(project_name, entries, expenses, week, year):
        ...
        if proj_expenses:
            ...
            for ex in proj_expenses:
                ...
                d_str = f"{d_nl} ({ex['Tarih']})" if e.get('Tarih') else d_nl # wait, e is not defined here, fix to ex
