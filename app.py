...
# Saat Slotu Üreteçleri (15'er dakika aralıklarla)
...
start_slots = generate_slots(3, 30, 10, 30) # 03:30 - 10:30
end_slots = generate_slots(12, 30, 19, 30)   # 12:30 - 19:30
...
with tab_entry:
    ...
    def update_worker_defaults(day_name, row_idx, wk_no):
        w_k = f"w_slide_{day_name}_{row_idx}_{wk_no}"
        s_k = f"s_slide_{day_name}_{row_idx}_{wk_no}"
        p_k = f"p_slide_{day_name}_{row_idx}_{wk_no}"
        e_k = f"e_slide_{day_name}_{row_idx}_{wk_no}"
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                p_key = f"p_slide_{day}_{i}_{week_no}"
                ...
                # Çalışan seçimi
                wrk = st.selectbox(f"Çalışan {i+1} ({day})", st.session_state.workers, key=w_key, on_change=update_worker_defaults, args=(day, i, week_no))
                
                st.markdown(f"⏱️ **Saatler - Çalışan {i+1}**")

                # --- BAŞLANGIÇ SAATİ (MANUEL TOGGLE YANINDA) ---
                ...
                if man_s:
                    final_start = st.text_input("Manuel Başlangıç", value=saved_e.get("Başlangıç", "07:00"), key=f"s_man_{day}_{i}_{week_no}", label_visibility="collapsed")
                else:
                    curr_s = st.session_state.get(s_key, "05:30" if init_w == "Adem" else "07:00")
                    ...
                    final_start = st.select_slider(
                        ...
                        options=start_slots, 
                        value=curr_s, 
                        key=f"s_sl_{day}_{i}_{week_no}",
                        label_visibility="collapsed"
                    )
                    st.session_state[s_key] = final_start

                # --- MOLA SAATİ (MANUEL TOGGLE YANINDA) ---
                ...
                if man_p:
                    final_pause = st.text_input("Manuel Mola", value=saved_e.get("Mola", "00:45"), key=f"p_man_{day}_{i}_{week_no}", label_visibility="collapsed")
                else:
                    curr_p = st.session_state.get(p_key, "01:45" if init_w == "Adem" else "00:45")
                    ...
                    final_pause = st.select_slider(
                        ...
                        options=pause_slots, 
                        value=curr_p, 
                        key=f"p_sl_{day}_{i}_{week_no}",
                        label_visibility="collapsed"
                    )
                    st.session_state[p_key] = final_pause

                # --- BİTİŞ SAATİ (MANUEL TOGGLE YANINDA) ---
                ...
                if man_e:
                    final_stop = st.text_input("Manuel Bitiş", value=saved_e.get("Bitiş", "15:45"), key=f"e_man_{day}_{i}_{week_no}", label_visibility="collapsed")
                else:
                    curr_e = st.session_state.get(e_key, "17:15" if init_w == "Adem" else "15:45")
                    ...
                    final_stop = st.select_slider(
                        ...
                        options=end_slots, 
                        value=curr_e, 
                        key=f"e_sl_{day}_{i}_{week_no}",
                        label_visibility="collapsed"
                    )
                    st.session_state[e_key] = final_stop
                
                calc_hours = calculate_net_hours(final_start, final_pause, final_stop)
                st.markdown(f"⏱️ **{wrk or 'Seçilmedi'}** | Net: **<span style='color:blue'>{calc_hours} saat</span>** ({final_start} - {final_stop}, Mola: {final_pause})", unsafe_allow_html=True)
