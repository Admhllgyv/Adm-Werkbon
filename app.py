    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                # --- SAATLER İÇİN NET METİN GÖSTEREN SELECT SLIDER ---
                st.caption(f"⏱️ Saatler - Çalışan {i+1}")
                
                start = st.select_slider(
                    f"Başlangıç {i+1}", 
                    options=start_slots, 
                    value=st.session_state[s_key] if st.session_state[s_key] in start_slots else "07:00", 
                    key=f"s_sl_{day}_{i}_{week_no}"
                )
                st.session_state[s_key] = start
                start_manual = st.text_input(f"M. Başlangıç", value=saved_e.get("Başlangıç", "07:00"), key=f"s_man_{day}_{i}_{week_no}")
                final_start = start_manual if start == "Manuel" else start

                pause = st.select_slider(
                    f"Mola {i+1}", 
                    options=pause_slots, 
                    value=st.session_state[p_key] if st.session_state[p_key] in pause_slots else "00:45", 
                    key=f"p_sl_{day}_{i}_{week_no}"
                )
                st.session_state[p_key] = pause
                pause_manual = st.text_input(f"M. Mola", value=saved_e.get("Mola", "00:45"), key=f"p_man_{day}_{i}_{week_no}")
                final_pause = pause_manual if pause == "Manuel" else pause

                stop = st.select_slider(
                    f"Bitiş {i+1}", 
                    options=end_slots, 
                    value=st.session_state[e_key] if st.session_state[e_key] in end_slots else "15:45", 
                    key=f"e_sl_{day}_{i}_{week_no}"
                )
                st.session_state[e_key] = stop
                stop_manual = st.text_input(f"M. Bitiş", value=saved_e.get("Bitiş", "15:45"), key=f"e_man_{day}_{i}_{week_no}")
                final_stop = stop_manual if stop == "Manuel" else stop
