...
# --- ÖZELLEŞTİRİLMİŞ SLİDER + MANUEL GİRİŞLİ SAAT SEÇİCİ ---
def slider_with_manual_time_picker(label, session_key, default_val="08:00", min_h=3, max_h=12, step_mins=15, disabled=False):
    st.markdown(f"<span style='font-size:12px; font-weight:bold; color:#1F4E78;'>{label}</span>", unsafe_allow_html=True)
    
    try:
        parts = default_val.split(":")
        def_mins = int(parts[0]) * 60 + int(parts[1])
    except Exception:
        def_mins = min_h * 60
        
    slider_key = f"slide_{session_key}"
    text_key = f"txt_{session_key}"
    state_val_key = f"val_{session_key}"
    
    if state_val_key not in st.session_state:
        st.session_state[state_val_key] = default_val
    ...
    current_str = st.session_state[state_val_key]
    current_mins = str_to_mins(current_str)

    min_val_mins = min_h * 60
    max_val_mins = max_h * 60
    
    slider_val = min(max(min_val_mins, current_mins), max_val_mins)
    ...
    with col_s:
        selected_mins = st.slider(
            label="Kaydırıcı",
            min_value=min_val_mins,
            max_value=max_val_mins,
            value=slider_val,
            step=step_mins,
            format="%d dk",
            key=slider_key,
        ...
        slider_str = mins_to_str(selected_mins)
    ...
    final_val = slider_str
    if manual_str != current_str:
        try:
            datetime.strptime(manual_str.strip(), "%H:%M")
            final_val = manual_str.strip()
        except Exception:
            pass
    elif slider_str != mins_to_str(current_mins):
        final_val = slider_str

    st.session_state[state_val_key] = final_val
    st.markdown(f"<div style='font-size:11px; color:#475569; margin-top:-4px;'>Seçilen: <b>{final_val}</b></div>", unsafe_allow_html=True)
    return final_val
                ...
                for p_i in range(int(num_projects)):
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            lock_key = f"lock_{day}{p_i}{w_i}_{week_no}"

                            saved_w_entry = proj_entries[w_i] if w_i < len(proj_entries) else None

                            if saved_w_entry and lock_key not in st.session_state:
                                st.session_state[lock_key] = saved_w_entry.get("Kilitli", False)
                            elif lock_key not in st.session_state:
                                st.session_state[lock_key] = False

                            is_entry_locked = st.session_state[lock_key] or is_week_locked
                            ...
                            if wrk != "Seçiniz":
                                ...
                                # --- 1) BAŞLANGIÇ: 3:00 - 12:00 (15 dk aralıklı) ---
                                final_start = slider_with_manual_time_picker(
                                    "Başlangıç Saati (03:00 - 12:00 arası / veya manuel)", 
                                    f"start_{day}{p_i}{w_i}_{week_no}", 
                                    st.session_state.get(s_key, def_s), 
                                    min_h=3, max_h=12, step_mins=15, 
                                    disabled=is_entry_locked
                                )
                                st.session_state[s_key] = final_start

                                # --- 2) MOLA: 0:00 - 2:00 (15 dk aralıklı) ---
                                final_pause = slider_with_manual_time_picker(
                                    "Mola Süresi (00:00 - 02:00 arası / veya manuel)", 
                                    f"pause_{day}{p_i}{w_i}_{week_no}", 
                                    st.session_state.get(pk_key, def_p), 
                                    min_h=0, max_h=2, step_mins=15, 
                                    disabled=is_entry_locked
                                )
                                st.session_state[pk_key] = final_pause

                                # --- 3) BİTİŞ: 11:00 - 20:00 (15 dk aralıklı) ---
                                final_stop = slider_with_manual_time_picker(
                                    "Bitiş Saati (11:00 - 20:00 arası / veya manuel)", 
                                    f"stop_{day}{p_i}{w_i}_{week_no}", 
                                    st.session_state.get(e_key, def_e), 
                                    min_h=11, max_h=20, step_mins=15, 
                                    disabled=is_entry_locked
                                )
                                ...
                                if not is_week_locked:
                                    ...
                                    with col_lk1:
                                        if is_entry_locked:
                                            if st.button("🔓 Saatleri Aç", key=f"unlock_{day}{p_i}{w_i}_{week_no}"):
                                                st.session_state[lock_key] = False
                                                st.rerun()
                                        else:
                                            if st.button("🔒 Saatleri Kilitle", key=f"lock_{day}{p_i}{w_i}_{week_no}"):
