...
st.markdown("""
    <style>
    /* MOBİL İÇİN KUSURSUZ TEK SATIR YATAY KAYDIRILABİLİR (SWIPE) ŞERİT TASARIMI */
    .horizontal-strip {
        display: flex;
        flex-direction: row;
        overflow-x: auto;
    ...
    .horizontal-strip::-webkit-scrollbar {
        display: none; /* Çirkin kaydırma çubuğunu gizle */
    }
...
# Yatay kaydırılabilir buton seçici fonksiyonu (Native Streamlit radio/selectbox yerine)
def swipeable_selector(label, options, current_val, key_prefix):
    st.caption(label)
    # Streamlit radio'yu yatay hale getirip CSS ile şerit yapıyoruz
    selected = st.radio(
        label,
        options,
...
with tab_preview:
    ...
    if "prev_y_sel" not in st.session_state: st.session_state["prev_y_sel"] = str(current_iso_year)
    if "prev_w_sel" not in st.session_state: st.session_state["prev_w_sel"] = str(current_iso_week)
    
    prev_sel_year_str = swipeable_selector("Yıl (Kaydırın ↔️)", years_list, st.session_state["prev_y_sel"], "prev_y")
    prev_sel_year = int(prev_sel_year_str) if prev_sel_year_str else current_iso_year
    
    prev_week_no_str = swipeable_selector("Hafta (Kaydırın ↔️)", weeks_list, st.session_state["prev_w_sel"], "prev_w")
...
with tab_entry:
    ...
    if "entry_y_sel" not in st.session_state: st.session_state["entry_y_sel"] = str(current_iso_year)
    if "entry_w_sel" not in st.session_state: st.session_state["entry_w_sel"] = str(current_iso_week)

    sel_year_str = swipeable_selector("Yıl (Kaydırın ↔️)", years_list, st.session_state["entry_y_sel"], "entry_y")
    sel_year = int(sel_year_str) if sel_year_str else current_iso_year
    
    week_no_str = swipeable_selector("Hafta Seçimi (Kaydırın ↔️)", weeks_list, st.session_state["entry_w_sel"], "entry_w")
    ...
    def update_worker_defaults(day_name, row_idx, wk_no):
        w_k = f"w_sel_{day_name}_{row_idx}_{wk_no}"
        s_k = f"s_sel_{day_name}_{row_idx}_{wk_no}"
        p_k = f"p_sel_{day_name}_{row_idx}_{wk_no}"
        e_k = f"e_sel_{day_name}_{row_idx}_{wk_no}"
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                w_key = f"w_sel_{day}_{i}_{week_no}"
                s_key = f"s_sel_{day}_{i}_{week_no}"
                p_key = f"p_sel_{day}_{i}_{week_no}"
                e_key = f"e_sel_{day}_{i}_{week_no}"
                ...
                wrk = swipeable_selector(f"Çalışan {i+1}", st.session_state.workers, st.session_state[w_key], w_key)
                if wrk != init_w:
                    update_worker_defaults(day, i, week_no)

                s_choice = swipeable_selector(f"Başlangıç {i+1} (Kaydırın ↔️)", start_slots, st.session_state[s_key], s_key)
                start_manual = st.text_input(f"M. Başlangıç", value=saved_e.get("Başlangıç", "07:00"), key=f"s_man_{day}_{i}_{week_no}")
                start = start_manual if s_choice == "Manuel" else s_choice

                p_choice = swipeable_selector(f"Mola {i+1} (Kaydırın ↔️)", pause_slots, st.session_state[p_key], p_key)
                pause_manual = st.text_input(f"M. Mola", value=saved_e.get("Mola", "00:45"), key=f"p_man_{day}_{i}_{week_no}")
                pause = pause_manual if p_choice == "Manuel" else p_choice

                e_choice = swipeable_selector(f"Bitiş {i+1} (Kaydırın ↔️)", end_slots, st.session_state[e_key], e_key)
                stop_manual = st.text_input(f"M. Bitiş", value=saved_e.get("Bitiş", "15:45"), key=f"e_man_{day}_{i}_{week_no}")
                stop = stop_manual if e_choice == "Manuel" else e_choice
