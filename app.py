...
# Mobile Safe Margin & Compact UI Custom CSS
st.markdown("""
    <style>
    .block-container {
        padding-left: 2rem;
    ...
    @media (max-width: 768px) {
        .block-container {
            padding-left: 1rem;
            padding-right: 1.5rem; /* Sağ elde kaydırma payı için ekstra boşluk */
        }
    }
    </style>
...
with tab_settings:
    st.header("⚙️ Firma ve Çalışan Ayarları")
...
with tab_preview:
    ...
    st.markdown("PDF indirmeden önce girilen saatleri, çalışan bazlı dağılımları ve masrafları buradan kontrol edebilirsiniz.")
    ...
    if all_entries:
        ...
        for proj in unique_projects:
            ...
            # Çalışan bazlı saat detay metriği
            w_str_summary = " | ".join([f"**{w}:** {hrs} saat" for w, hrs in worker_hours.items()])
            st.info(f"👥 **Çalışan Dağılımı:** {w_str_summary}")
...
def create_pdf(project_name, entries, expenses, week):
    ...
    # Expenses Table in Dutch with Codes
    ...
    # Receipt Images Appendix with Watermarked / Drawn Code Banner
    ...
    if receipts_with_images:
        ...
        for ex in receipts_with_images:
            story.append(Paragraph(f"<b>{ex['Kod']} - {ex['Gün']} - {ex['Açıklama']} (€{ex['Tutar']:.2f}):</b>", sub_style))
            story.append(Spacer(1, 4))
            try:
                ex["Görsel"].seek(0)
                pil_rec = PILImage.open(ex["Görsel"]).convert("RGB")
                
                # Resim üzerine kod etiketi (banner) ekleme
                draw = ImageDraw.Draw(pil_rec)
                w_img, h_img = pil_rec.size
                
                # Üst köşeye şık bir etiket çizelim
                banner_height = int(h_img * 0.08)
                if banner_height < 30: banner_height = 30
                
                # Siyah yarı saydam veya kırmızı/mavi şerit yerine net bir kutu
                draw.rectangle([(0, 0), (w_img, banner_height)], fill=(31, 78, 120))
                
                # Metin ekleme (varsayılan font sorunsuz çalışması için)
                try:
                    font = ImageFont.truetype("arial.ttf", int(banner_height * 0.6))
                except IOError:
                    font = ImageFont.load_default()
                    
                draw.text((15, banner_height * 0.15), f"{ex['Kod']} | {ex['Gün']} | €{ex['Tutar']:.2f}", fill=(255, 255, 255), font=font)
