...
TARGET_ANDROID_BACKUP_DIR = "/storage/emulated/0/0.Adm Technics Otomasyon"
...
with tab_settings:
    ...
    st.info(f"📂 **Otomatik Yedekleme Yolu:** Her Cumartesi saat 02:00'de şu klasöre yedek alınır:\n`{TARGET_ANDROID_BACKUP_DIR}`")
    ...
    if st.button("🚀 Şimdi Test Yedeği Al ve Klasörü Kontrol Et"):
        try:
            ...
            with open(test_path, "w", encoding="utf-8") as f:
                json.dump(export_data, f, ensure_ascii=False)
            st.success(f"Başarılı! Test yedeği bu konuma kaydedildi:\n`{test_path}`")
        except Exception as e:
            st.error(f"Hata oluştu (Android depolama izni veya klasör yolu erişimi kısıtlı olabilir):\n{e}")
