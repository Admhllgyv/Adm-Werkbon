...
st.markdown("""
    ...
    /* Tabloların düzgün görünmesi ve harf bölünmesinin önlenmesi */
    .table-container {
        width: 100%;
        overflow-x: auto;
    }
...
# Klavye açılmasını önlemek için 5 adedin altında tutulan optimize slotlar
start_slots = ["05:30", "06:00", "07:00", "07:30", "Manuel"]
end_slots = ["15:00", "15:45", "16:00", "17:15", "Manuel"]
pause_slots = ["00:30", "00:45", "01:00", "01:45", "Manuel"]
...
with tab_preview:
    ...
    with col_pwk:
        prev_week_no = st.slider("Hafta No", min_value=1, max_value=53, value=current_iso_week, key="prev_w")
...
with tab_entry:
    ...
    with col_wk:
        is_completed = f"{sel_year}-W{current_iso_week}" in st.session_state.completed_weeks
        week_no = st.slider("Hafta Seçimi", min_value=1, max_value=53, value=current_iso_week)
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                # Çalışanlar arasına gri ayırıcı çizgi (İşçiler ile diğer alanları ayıran yapı)
                if i > 0:
