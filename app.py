    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                def get_initial_idx(saved_val, default_adem, default_other, valid_slots):
                    val = saved_val if saved_val else (default_adem if init_w == "Adem" else default_other)
                    return valid_slots.index(val) if val in valid_slots else 0

                if s_key not in st.session_state:
                    st.session_state[s_key] = get_initial_idx(saved_e.get("Başlangıç"), "05:30", "07:00", start_slots)
                if p_key not in st.session_state:
                    st.session_state[p_key] = get_initial_idx(saved_e.get("Mola"), "01:45", "00:45", pause_slots)
                if e_key not in st.session_state:
                    st.session_state[e_key] = get_initial_idx(saved_e.get("Bitiş"), "17:15", "15:45", end_slots)
                ...
                # --- SAATLER İÇİN KAYDIRMA ÇUBUĞU (SLIDER) ---
                st.caption(f"⏱️ Saatler - Çalışan {i+1}")
                s_idx = st.slider(f"Başlangıç {i+1}", 0, len(start_slots)-1, st.session_state[s_key], key=f"s_sl_{day}_{i}_{week_no}", format="")
                s_choice = start_slots[s_idx]
                ...
                p_idx = st.slider(f"Mola {i+1}", 0, len(pause_slots)-1, st.session_state[p_key], key=f"p_sl_{day}_{i}_{week_no}", format="")
                p_choice = pause_slots[p_idx]
                ...
                e_idx = st.slider(f"Bitiş {i+1}", 0, len(end_slots)-1, st.session_state[e_key], key=f"e_sl_{day}_{i}_{week_no}", format="")
                e_choice = end_slots[e_idx]
                ...
                st.markdown(f"⏱️ **{wrk or 'Seçilmedi'}** | Net: **<span style='color:blue'>{calc_hours} saat</span>** ({start} - {stop}, Mola: {pause})", unsafe_allow_html=True)
