...
def time_picker_component_unified(label, min_h, max_h, session_key, default_val, disabled=False):
    ...
    if is_manual:
        ...
        # Eğer dışarıdan gelen default_val session_state'deki değerden farklıysa veya yoksa güncelle
                ...
                for p_i in range(int(num_projects)):
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            # Radyo butonlarının oturum anahtarları (session state state sync)
                            s_session_key = f"s_{day}{p_i}{w_i}_{week_no}"
                            p_session_key = f"p_{day}{p_i}{w_i}_{week_no}"
                            e_session_key = f"e_{day}{p_i}{w_i}_{week_no}"

                            t_start_key = f"t_{s_session_key}"
                            b_pause_key = f"b_{p_session_key}"
                            t_end_key = f"t_{e_session_key}"

                            # Çalışan değiştiğinde veya ilk defa yüklendiğinde varsayılan saatleri radyo butonuna zorla ata
                            if prev_w != wrk or t_start_key not in st.session_state:
                                st.session_state[t_start_key] = def_s
                                st.session_state[b_pause_key] = def_p
                                st.session_state[t_end_key] = def_e
                                st.session_state[prev_w_key] = wrk

                            if wrk != "Seçiniz":
                                st.markdown(f"⏱️ Saatler - {wrk}")

                                final_start = time_picker_component_unified("Başlangıç (03:00 - 12:00)", 3, 12, s_session_key, def_s, disabled=is_week_locked)
                                final_pause = break_picker_component("Mola", p_session_key, def_p, disabled=is_week_locked)
                                final_stop = time_picker_component_unified("Bitiş (11:00 - 20:00)", 11, 20, e_session_key, def_e, disabled=is_week_locked)
