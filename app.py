...
with tab_settings:
    st.header("⚙️ Ayarlar ve Veri Güvenliği")
    ...
    try:
        ...
        with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
            ...
            for wk_k, wk_val in st.session_state.weekly_data.items():
                for exp in wk_val.get("expenses", []):
                    all_flat_expenses.append({"Hafta": wk_k, **exp})
        ...
        st.info("Excel yedekleme hazırlanıyor...")
...
with tab_entry:
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            num_projects = st.number_input(f"Günlük Proje / Görev Sayısı ({day})", min_value=0, max_value=5, value=max(1, len(d_entries)), key=f"num_proj_{day}_{week_no}")
            ...
            for p_i in range(int(num_projects)):
                ...
                saved_e = d_entries[p_i] if p_i < len(d_entries) else {}
                p_key = f"proj_sel_{day}_{p_i}_{week_no}"
                
                # 1. Proje Seçimi
                if p_key not in st.session_state:
                    saved_p = saved_e.get("Proje", "Seçiniz")
                    st.session_state[p_key] = saved_p if saved_p in project_options else "Seçiniz"
                
                cur_proj = st.session_state.get(p_key, "Seçiniz")
                if cur_proj not in project_options:
                    cur_proj = "Seçiniz"
                p_idx = project_options.index(cur_proj)
                
                proj_sel = st.selectbox(f"Proje {p_i+1} ({day})", project_options, index=p_idx, key=p_key)

                if proj_sel != "Seçiniz":
                    st.markdown(f"#### 📁 {proj_sel}")
                    
                    num_workers = st.number_input(f"Çalışan Sayısı ({proj_sel} - {day})", min_value=0, max_value=5, value=1, key=f"num_w_{day}_{p_i}_{week_no}")
                    ...
                    for w_i in range(int(num_workers)):
                        ...
                        if w_key not in st.session_state:
                            st.session_state[w_key] = saved_e.get("Çalışan", "Seçiniz") if w_i == 0 else "Seçiniz"
                        ...
                        wrk = st.selectbox(f"Çalışan {w_i+1} ({proj_sel})", worker_options, index=w_idx, key=w_key)

                        # Otomatik saatler (Adem vs Diğerleri)
                        if wrk == "Adem":
                            def_s, def_p, def_e = "05:30", "01:45", "17:15"
                        else:
                            def_s, def_p, def_e = "07:00", "00:45", "15:45"

                        prev_wrk = st.session_state.get(prev_w_key, "Seçiniz")
                        if s_key not in st.session_state or prev_wrk != wrk:
                            st.session_state[s_key] = saved_e.get("Başlangıç", def_s) if prev_wrk == wrk and saved_e else def_s
                            st.session_state[pk_key] = saved_e.get("Mola", def_p) if prev_wrk == wrk and saved_e else def_p
                            st.session_state[e_key] = saved_e.get("Bitiş", def_e) if prev_wrk == wrk and saved_e else def_e
                        ...
                        if wrk != "Seçiniz":
                            ...
                            with col_s2:
                                man_s = st.checkbox("Manuel", key=f"man_s_{day}_{p_i}_{w_i}_{week_no}")
                            
                            if man_s:
                                final_start = st.text_input("Manuel Başlangıç", value=st.session_state[s_key], key=f"s_man_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed")
                            else:
                                curr_s = st.session_state.get(s_key, def_s)
                                if curr_s not in start_slots:
                                    curr_s = start_slots[0]
                                final_start = st.select_slider(
                                    f"Başlangıç {w_i+1}", options=start_slots, value=curr_s, key=f"s_sl_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed"
                                )
                            ...
                            with col_p1:
                                st.markdown(f"**Mola**")
                            with col_p2:
                                man_p = st.checkbox("Manuel", key=f"man_p_{day}_{p_i}_{w_i}_{week_no}")

                            if man_p:
                                final_pause = st.text_input("Manuel Mola", value=st.session_state[pk_key], key=f"p_man_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed")
                            else:
                                curr_p = st.session_state.get(pk_key, def_p)
                                if curr_p not in pause_slots:
                                    curr_p = pause_slots[0]
                                final_pause = st.select_slider(
                                    f"Mola {w_i+1}", options=pause_slots, value=curr_p, key=f"p_sl_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed"
                                )
                            ...
                            with col_e1:
                                st.markdown(f"**Bitiş**")
                            with col_e2:
                                man_e = st.checkbox("Manuel", key=f"man_e_{day}_{p_i}_{w_i}_{week_no}")

                            if man_e:
                                final_stop = st.text_input("Manuel Bitiş", value=st.session_state[e_key], key=f"e_man_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed")
                            else:
                                curr_e = st.session_state.get(e_key, def_e)
                                if curr_e not in end_slots:
                                    curr_e = end_slots[0]
                                final_stop = st.select_slider(
                                    f"Bitiş {w_i+1}", options=end_slots, value=curr_e, key=f"e_sl_{day}_{p_i}_{w_i}_{week_no}", label_visibility="collapsed"
                                )
                    ...
                    # Proje Bazlı Fiş / Gider Bölümü
                    st.markdown(f"💶 **Fiş / Gider ({proj_sel})**")
                    num_exp = st.number_input(f"Fiş Sayısı ({proj_sel})", min_value=0, max_value=5, value=0, key=f"num_exp_{day}_{p_i}_{week_no}")
                    for j in range(int(num_exp)):
                        e_cols = st.columns([2, 1.5, 2.5])
                        with e_cols[0]:
                            exp_desc = st.selectbox("Gider", st.session_state.expense_categories, key=f"exp_desc_{day}_{p_i}_{j}_{week_no}", label_visibility="collapsed")
                        with e_cols[1]:
                            exp_amount = st.number_input("Tutar", min_value=0.0, value=10.0, step=1.0, key=f"exp_amt_{day}_{p_i}_{j}_{week_no}", label_visibility="collapsed")
                        with e_cols[2]:
