    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                # 1. Önce Proje Seçimi ("Seçiniz" ile başlar)
                ...
                # Eğer proje seçildiyse hemen altında çalışan seçimi çıksın
                if day_proj != "Seçiniz":
                    if w_key not in st.session_state:
                        saved_w = saved_e.get("Çalışan", "Seçiniz")
                        st.session_state[w_key] = saved_w if saved_w in worker_options else "Seçiniz"
                    ...
                    # Çalışan değiştiğinde o çalışana ait varsayılan saatleri güvenle güncelleyen callback
                    def worker_changed_callback():
                        selected_w = st.session_state.get(w_key)
                        if selected_w == "Adem":
                            st.session_state[s_key] = "05:30"
                            st.session_state[pk_key] = "01:45"
                            st.session_state[e_key] = "17:15"
                        elif selected_w and selected_w != "Seçiniz":
                            st.session_state[s_key] = "07:00"
                            st.session_state[pk_key] = "00:45"
                            st.session_state[e_key] = "15:45"

                    wrk = st.selectbox(f"Çalışan {i+1} ({day})", worker_options, index=w_idx, key=w_key, on_change=worker_changed_callback)

                    # Otomatik saatlerin başlatılması
                    default_s = "05:30" if wrk == "Adem" else "07:00"
                    default_p = "01:45" if wrk == "Adem" else "00:45"
                    default_e = "17:15" if wrk == "Adem" else "15:45"
