...
# --- HIZLANDIRILMIŞ FRAGMENT BİLEŞENİ (@st.fragment) ---
@st.fragment
def compact_time_picker_component(label, session_key, default_val="08:00", min_h=0, max_h=23, disabled=False):
    st.markdown(f"<span style='font-size:11px; font-weight:bold; color:#1F4E78;'>{label}</span>", unsafe_allow_html=True)
    
    try:
        def_h, def_m = map(int, default_val.split(":"))
    except Exception:
        def_h, def_m = 8, 0
        
    if def_h < min_h or def_h > max_h:
        def_h = 8
    if def_m not in [0, 15, 30, 45]:
        def_m = 0
        
    h_key = f"h_{session_key}"
    m_key = f"m_{session_key}"
    
    if h_key not in st.session_state or st.session_state[h_key] not in range(min_h, max_h + 1):
        st.session_state[h_key] = def_h
    if m_key not in st.session_state or st.session_state[m_key] not in [0, 15, 30, 45]:
        st.session_state[m_key] = def_m

    c_h_col, c_m_col, c_info_col = st.columns([2, 2, 2])
    
    with c_h_col:
        st.markdown("<span style='font-size:10px; color:#475569;'>Saat</span>", unsafe_allow_html=True)
        hc1, hc2, hc3 = st.columns([1, 1.5, 1])
        if hc1.button("➖", key=f"h_minus_{session_key}", disabled=disabled):
            st.session_state[h_key] = (st.session_state[h_key] - 1) % 24
        with hc2:
            st.markdown(f"<div style='text-align:center; font-weight:bold; font-size:13px; padding-top:4px;'>{st.session_state[h_key]:02d}</div>", unsafe_allow_html=True)
        if hc3.button("➕", key=f"h_plus_{session_key}", disabled=disabled):
            st.session_state[h_key] = (st.session_state[h_key] + 1) % 24
            
    with c_m_col:
        st.markdown("<span style='font-size:10px; color:#475569;'>Dakika (15dk)</span>", unsafe_allow_html=True)
        mc1, mc2, mc3 = st.columns([1, 1.5, 1])
        min_list = [0, 15, 30, 45]
        curr_m_idx = min_list.index(st.session_state[m_key]) if st.session_state[m_key] in min_list else 0
        
        if mc1.button("➖", key=f"m_minus_{session_key}", disabled=disabled):
            new_idx = (curr_m_idx - 1) % len(min_list)
            st.session_state[m_key] = min_list[new_idx]
        with mc2:
            st.markdown(f"<div style='text-align:center; font-weight:bold; font-size:13px; padding-top:4px;'>{st.session_state[m_key]:02d}</div>", unsafe_allow_html=True)
        if mc3.button("➕", key=f"m_plus_{session_key}", disabled=disabled):
            new_idx = (curr_m_idx + 1) % len(min_list)
            st.session_state[m_key] = min_list[new_idx]
            
    with c_info_col:
        st.markdown("<span style='font-size:10px; color:#475569;'>Seçilen</span>", unsafe_allow_html=True)
        st.markdown(f"<div style='padding-top:4px; font-size:12px; font-weight: bold; color: #2563EB;'>{st.session_state[h_key]:02d}:{st.session_state[m_key]:02d}</div>", unsafe_allow_html=True)
        
    return f"{st.session_state[h_key]:02d}:{st.session_state[m_key]:02d}"
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                # Hızlandırılmış Kompakt Saat Seçiciler
                                final_start = compact_time_picker_component("Başlangıç Saati", f"start_{day}{p_i}{w_i}_{week_no}", st.session_state.get(s_key, def_s), disabled=is_week_locked)
                                st.session_state[s_key] = final_start

                                final_pause = compact_time_picker_component("Mola Süresi", f"pause_{day}{p_i}{w_i}_{week_no}", st.session_state.get(pk_key, def_p), disabled=is_week_locked)
                                st.session_state[pk_key] = final_pause

                                final_stop = compact_time_picker_component("Bitiş Saati", f"stop_{day}{p_i}{w_i}_{week_no}", st.session_state.get(e_key, def_e), disabled=is_week_locked)
