...
st.markdown("""
    <style>
    /* Mobil ve dar ekranlarda sütunların alt alta kaymasını kesin olarak engelle ve yan yana tut */
...
with tab_settings:
    ...
    if logo_io is not None:
        st.image(logo_io, width=80, caption="Mevcut Logo")
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                # --- KESİN YAN YANA (YATAY) DÜZEN ---
                cols = st.columns([1.6, 2.8, 2.4, 2.8])
                with cols[0]:
                    wrk = st.selectbox(f"Kişi {i+1}", w_list, key=w_key, on_change=update_worker_defaults, args=(day, i, week_no), label_visibility="collapsed")
                    if not wrk:
                        st.caption("Kişi seç")
                with cols[1]:
