...
<script>
// SessionStorage Destekli Kaydırma Hafızası Motoru
function setupScrollPersistence() {
    const containers = document.querySelectorAll('div[data-testid="stRadio"] > div');
    
    containers.forEach((container, index) => {
        const storageKey = 'timeScrollPos_' + index;
        
        // Sayfa yüklenirken hafızadaki pozisyonu geri yükle
        const savedPos = sessionStorage.getItem(storageKey);
        if (savedPos !== null) {
            container.scrollLeft = parseFloat(savedPos);
        } else {
            // Hafızada yoksa seçili olan elemanı ortala
            const checkedInput = container.querySelector('input[type="radio"]:checked');
            if (checkedInput) {
                const label = checkedInput.closest('label');
                if (label) {
                    const targetLeft = label.offsetLeft - (container.clientWidth / 2) + (label.clientWidth / 2);
                    container.scrollLeft = targetLeft;
                }
            }
        }
        
        // Kullanıcı kaydırdıkça pozisyonu anında hafızaya kaydet
...
// Streamlit rerun döngülerinde ve yüklenmelerde çalıştır
window.addEventListener('DOMContentLoaded', setupScrollPersistence);
[50, 150, 300, 600].forEach(t => setTimeout(setupScrollPersistence, t));
