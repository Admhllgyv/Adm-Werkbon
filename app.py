...
/* Butonları ve + / - alanlarını mobilde daha büyük ve kolay tıklanabilir yapma */
.stButton button {
...
# Saatler İçin Güvenli ve Büyük Butonlu Stepper Bileşeni (+ / -)
def time_stepper(label, slots_list, session_key, default_val, disabled=False):
    if session_key not in st.session_state or st.session_state.get(f"init_{session_key}") != default_val:
        st.session_state[session_key] = default_val if default_val in slots_list else slots_list[0]
        st.session_state[f"init_{session_key}"] = default_val
        
    curr_val = st.session_state[session_key]
    if curr_val not in slots_list:
        curr_val = slots_list[0]
        st.session_state[session_key] = curr_val
        
    idx = slots_list.index(curr_val)
    
    st.markdown(f"<span style='font-size:12px; font-weight:600; color:#475569;'>{label}</span>", unsafe_allow_html=True)
    col_m, col_t, col_p = st.columns([1, 2, 1])
    with col_m:
        if st.button("➖", key=f"btn_m_{session_key}", disabled=disabled, use_container_width=True):
            if idx > 0:
                st.session_state[session_key] = slots_list[idx - 1]
                st.rerun()
    with col_t:
        st.markdown(f"<div style='text-align:center; font-size:15px; font-weight:bold; padding:8px 0; background:#F8FAFC; border: 1px solid #CBD5E1; border-radius:6px;'>{st.session_state[session_key]}</div>", unsafe_allow_html=True)
    with col_p:
        if st.button("➕", key=f"btn_p_{session_key}", disabled=disabled, use_container_width=True):
            if idx < len(slots_list) - 1:
                st.session_state[session_key] = slots_list[idx + 1]
                st.rerun()
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                if man_s:
                                    ...
                                    final_start = time_stepper("Başlangıç Saati", start_slots, f"s_step{day}{p_i}{w_i}_{week_no}", curr_s, disabled=is_week_locked)
                                ...
                                if man_p:
                                    ...
                                    final_pause = time_stepper("Mola Süresi", pause_slots, f"p_step{day}{p_i}{w_i}_{week_no}", curr_p, disabled=is_week_locked)
                                ...
                                if man_e:
                                    ...
                                    final_stop = time_stepper("Bitiş Saati", end_slots, f"e_step{day}{p_i}{w_i}_{week_no}", curr_e, disabled=is_week_locked)
