...
with tab_settings:
    ...
    # Tek Tıkla Excel Raporu İndirme Butonu
    all_weeks_entries = []
    all_weeks_expenses = []
    ...
    if all_weeks_entries or all_weeks_expenses:
        try:
            ...
            # openpyxl yüklü değilse alternatif basit CSV/Excel yaklaşımı
            pass
...
with tab_projects:
    ...
    st.markdown("Buradan projelerinizi güncelleyebilir ve hangi projede kimin ne kadar çalıştığını inceleyebilirsiniz[span_2](start_span)[span_2](end_span).")
    ...
    for idx, prj in enumerate(sorted_projects):
        with st.expander(f"📌 {prj['code']} - {prj['name']}"):
            ...
            if matched_entries:
                ...
                st.dataframe(df_m[["Hafta", "Gün", "Tarih", "Çalışan", "Başlangıç", "Mola", "Bitiş", "Saat"]], use_container_width=True)
            else:
            ...
            if matched_expenses:
                ...
                st.dataframe(df_me[["Hafta", "Gün", "Tarih", "Kod", "Açıklama", "Tutar"]], use_container_width=True)
