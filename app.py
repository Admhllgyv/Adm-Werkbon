...
# Android izin engeline takılmamak için en güvenli yol olan Download klasörü:
TARGET_ANDROID_BACKUP_DIR = "/storage/emulated/0/Download/Adm_Technics_Yedek"
    ...
    while True:
        try:
            ...
            if now.weekday() == 5 and now.hour == 2 and now.minute == 0:
                os.makedirs(TARGET_ANDROID_BACKUP_DIR, exist_ok=True)
                backup_filename = f"adm_technics_otomatik_yedek_{now.strftime('%Y%m%d_%H%M')}.json"
                backup_path = os.path.join(TARGET_ANDROID_BACKUP_DIR, backup_filename)
...
with tab_settings:
    ...
    st.info(f"📂 **Otomatik Yedekleme Yolu (İndirilenler):** Her Cumartesi saat 02:00'de şu klasöre yedek alınır:\n`{TARGET_ANDROID_BACKUP_DIR}`")
    ...
    if st.button("🚀 Şimdi Test Yedeği Al ve Klasörü Kontrol Et"):
        try:
            ...
            test_path = os.path.join(TARGET_ANDROID_BACKUP_DIR, test_filename)
            ...
            with open(test_path, "w", encoding="utf-8") as f:
                json.dump(export_data, f, ensure_ascii=False)
            st.success(f"Başarılı! Test yedeği telefonun İndirilenler klasörüne kaydedildi:\n`{test_path}`")
