...
/* Sayfa bütünüyle kaymasın, yalnızca iç şerit kaydırılsın */
...
<script>
// Seçili olan çipi kesin olarak yatay şeridin tam ortasına getiren kusursuz koordinat hesaplama fonksiyonu
...
// Streamlit yüklenme ve yeniden çizilme (rerun) döngülerini yakalamak için kademeli tetikleme
[50, 150, 300, 600, 1000, 1500].forEach(delay => {
    setTimeout(centerActiveChips, delay);
});

// Kullanıcı seçim yaptığında veya dokunduğunda anında ortala
document.addEventListener('change', (e) => {
    if (e.target && (e.target.type === 'radio' || e.target.type === 'checkbox')) {
        setTimeout(centerActiveChips, 100);
        setTimeout(centerActiveChips, 400);
    }
});
</script>
