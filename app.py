...
WEEKLY_AUTO_BACKUP_FILE = os.path.join(TARGET_BACKUP_DIR, "otomatik_yedek.json")

# --- OTOMATİK HAFTALIK ARKA PLAN YEDEKLEME (CUMARTESİ SAAT 02:00 - ÜZERİNE YAZMali) ---
    ...
    while True:
        try:
            ...
            if now.weekday() == 5 and now.hour == 2 and now.minute == 0:
                ...
                with open(WEEKLY_AUTO_BACKUP_FILE, "w", encoding="utf-8") as f:
                    json.dump(export_data, f, ensure_ascii=False)
                time.sleep(60) # Aynı dakika içinde tekrar tetiklenmemesi için 1 dakika bekle
        except Exception:
            pass
        time.sleep(30)
...
with tab_settings:
    st.header("⚙️ Ayarlar ve Otomatik Güvenlik & Geri Yükleme")
    
    st.subheader("🕒 Otomatik Haftalık Yedekleme Durumu")
    st.info(f"📂 Klasör: `{TARGET_BACKUP_DIR}/`\nHer Cumartesi saat 02:00'de bu klasördeki `otomatik_yedek.json` dosyası üzerine yazılır.")
    
    # --- KONTROL MEKANİZMASI ---
    if os.path.exists(WEEKLY_AUTO_BACKUP_FILE):
        file_mod_time = datetime.fromtimestamp(os.path.getmtime(WEEKLY_AUTO_BACKUP_FILE)).strftime('%d.%m.%Y %H:%M:%S')
        file_size_kb = round(os.path.getsize(WEEKLY_AUTO_BACKUP_FILE) / 1024, 2)
        st.success(f"✅ **Otomatik Yedek Aktif ve Mevcut!**\n- **Dosya Yolu:** `{WEEKLY_AUTO_BACKUP_FILE}`\n- **Son Güncelleme / Alınma Zamanı:** {file_mod_time}\n- **Dosya Boyutu:** {file_size_kb} KB")
    else:
        st.warning("⚠️ Henüz otomatik yedek dosyası oluşturulmamış (İlk Cumartesi saat 02:00'yi bekliyor veya aşağıdaki test butonunu kullanabilirsiniz).")

    col_bak1, col_bak2 = st.columns(2)
    with col_bak1:
        if st.button("🚀 Şimdi Test Yedeği Al (Hemen Oluştur)"):
            try:
                os.makedirs(TARGET_BACKUP_DIR, exist_ok=True)
                export_data = {
                    "comp_name": st.session_state.get("comp_name", "ADM TECHNICS"),
                ...
                with open(WEEKLY_AUTO_BACKUP_FILE, "w", encoding="utf-8") as f:
                    json.dump(export_data, f, ensure_ascii=False)
                st.success(f"Başarılı! Test yedeği kaydedildi:\n{WEEKLY_AUTO_BACKUP_FILE}")
                st.rerun()
            except Exception as e:
                st.error(f"Hata oluştu:\n{e}")

    with col_bak2:
        if st.button("🔄 Otomatik Yedeği Geri Yükle (%100 Geri Al)"):
            if os.path.exists(WEEKLY_AUTO_BACKUP_FILE):
                try:
                    with open(WEEKLY_AUTO_BACKUP_FILE, "r", encoding="utf-8") as f:
                        imported_data = json.load(f)
                    ...
                    st.success("Otomatik yedek başarıyla ve eksiksiz (%100) geri yüklendi!")
                    st.rerun()
                except Exception as e:
                    st.error(f"Geri yükleme sırasında hata oluştu: {e}")
            else:
                st.error("Geri yüklenecek otomatik yedek dosyası bulunamadı!")
    ...
    uploaded_backup = st.file_uploader("📂 Manuel Yedek Geri Yükle", type=["json"])
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                final_start = st.select_slider(f"Başlangıç {w_i+1}", options=start_slots, value=curr_s, key=s_key)

                                curr_p = st.session_state.get(pk_key, def_p)
                                if curr_p not in pause_slots:
                                    curr_p = pause_slots[3]
                                final_pause = st.select_slider(f"Mola {w_i+1}", options=pause_slots, value=curr_p, key=pk_key)

                                curr_e = st.session_state.get(e_key, def_e)
                                if curr_e not in end_slots:
                                    curr_e = end_slots[-1]
                                final_end = st.select_slider(f"Bitiş {w_i+1}", options=end_slots, value=curr_e, key=e_key)

                                net_hrs = calculate_net_hours(final_start, final_pause, final_end)
                                st.info(f"Hesaplanan Net Çalışma Süresi: **{net_hrs} Saat**")
