...
# --- KLAVYE AÇMAYAN BUTON TABANLI SAAT SEÇİCİ (SEGMENTED CONTROL) ---
def time_picker_component_unified(label, min_h, max_h, session_key, default_val, disabled=False):
    ...
    try:
        def_h, def_m = default_val.split(":")
        if int(def_h) < min_h or int(def_h) > max_h:
            def_h = f"{min_h:02d}"
    except Exception:
        def_h, def_m = f"{min_h:02d}", "00"
        
    hours_list = [f"{h:02d}" for h in range(min_h, max_h + 1)]
    minutes_list = ["00", "15", "30", "45"]
    
    h_key = f"h_ctrl_{session_key}"
    m_key = f"m_ctrl_{session_key}"
    
    if h_key not in st.session_state or st.session_state[h_key] not in hours_list:
        st.session_state[h_key] = def_h if def_h in hours_list else hours_list[0]
    if m_key not in st.session_state or st.session_state[m_key] not in minutes_list:
        st.session_state[m_key] = def_m if def_m in minutes_list else "00"
        
    col1, col2 = st.columns(2)
    with col1:
        sel_h = st.segmented_control("Saat", hours_list, key=h_key, disabled=disabled, selection_mode="single")
    with col2:
        sel_m = st.segmented_control("Dakika", minutes_list, key=m_key, disabled=disabled, selection_mode="single")
        
    final_h = sel_h if sel_h else st.session_state[h_key]
    final_m = sel_m if sel_m else st.session_state[m_key]
    
    return f"{final_h}:{final_m}"
...
def break_picker_component(label, session_key, default_val, disabled=False):
    ...
    sel_break = st.segmented_control(label, break_options, key=b_key, disabled=disabled, selection_mode="single")
    return sel_break if sel_break else st.session_state[b_key]
