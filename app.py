...
with tab_preview:
    ...
    if w_data["entries"]:
        ...
        # Ekrana tam sığan, taşmayan HTML tablo yapısı
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=False):
            ...
            for i in range(int(num_rows)):
                ...
                # --- ÇALIŞAN ÜSTTE, SAATLER (BAŞLANGIÇ, MOLA, BİTİŞ) ALTINDA YAN YANA 3 SÜTUN ---
                wrk = st.selectbox(f"Çalışan {i+1} ({day})", w_list, key=w_key, on_change=update_worker_defaults, args=(day, i, week_no))
                
                time_cols = st.columns(3)
                with time_cols[0]:
                    s_choice = st.selectbox(f"Baş {i+1}", start_slots, key=s_key)
                    start = st.text_input(f"M.Baş {i+1}", value=saved_e.get("Başlangıç", "07:00"), key=f"s_man_{day}_{i}_{week_no}") if s_choice == "Manuel" else s_choice
                with time_cols[1]:
                    p_choice = st.selectbox(f"Mol {i+1}", pause_slots, key=p_key)
                    pause = st.text_input(f"M.Mol {i+1}", value=saved_e.get("Mola", "00:45"), key=f"p_man_{day}_{i}_{week_no}") if p_choice == "Manuel" else p_choice
                with time_cols[2]:
                    e_choice = st.selectbox(f"Bit {i+1}", end_slots, key=e_key)
                    stop = st.text_input(f"M.Bit {i+1}", value=saved_e.get("Bitiş", "15:45"), key=f"e_man_{day}_{i}_{week_no}") if e_choice == "Manuel" else e_choice
