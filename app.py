...
# --- 15 DAKİKA ARALIKLI SLİDER + MANUEL GİRİŞ DESTEKLİ SAAT SEÇİCİ ---
def slider_with_manual_time_picker(label, session_key, default_val="08:00", disabled=False):
    ...
    with col_s:
        selected_mins = st.slider(
            ...
            min_value=0,
            max_value=1425,
            value=min(max(0, current_mins), 1425),
            step=15,
            format="%d dk",
            key=slider_key,
            disabled=disabled,
    ...
    st.markdown(f"<div style='font-size:11px; color:#475569; margin-top:-4px;'>Seçilen Saat: <b>{final_val}</b></div>", unsafe_allow_html=True)
    return final_val
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                # Kaydırıcı + Manuel Giriş Destekli Saat Seçiciler
                                final_start = slider_with_manual_time_picker("Başlangıç Saati (15 dk adım / Manuel)", f"start_{day}{p_i}{w_i}_{week_no}", st.session_state.get(s_key, def_s), disabled=is_week_locked)
                                st.session_state[s_key] = final_start

                                final_pause = slider_with_manual_time_picker("Mola Süresi", f"pause_{day}{p_i}{w_i}_{week_no}", st.session_state.get(pk_key, def_p), disabled=is_week_locked)
                                st.session_state[pk_key] = final_pause

                                final_stop = slider_with_manual_time_picker("Bitiş Saati", f"stop_{day}{p_i}{w_i}_{week_no}", st.session_state.get(e_key, def_e), disabled=is_week_locked)
