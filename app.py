            ...
            for p_i in range(int(num_projects)):
                ...
                if proj_sel != "Seçiniz":
                    ...
                    for w_i in range(int(num_workers)):
                        ...
                        s_key = f"s_sl_{day}_{p_i}_{w_i}_{week_no}"
                        pk_key = f"p_sl_{day}_{p_i}_{w_i}_{week_no}"
                        e_key = f"e_sl_{day}_{p_i}_{w_i}_{week_no}"
                        ...
                        # Slider Session State Tanımlamaları
                        ...
                        if wrk != "Seçiniz":
                            ...
                            if curr_s not in start_slots: curr_s = start_slots[0]
                            final_start = st.select_slider(
                                f"Başlangıç {w_i+1}", options=start_slots, value=curr_s, key=s_key, disabled=is_locked
                            )

                            curr_p = st.session_state.get(pk_key, def_p)
                            if curr_p not in pause_slots: curr_p = pause_slots[0]
                            final_pause = st.select_slider(
                                f"Mola {w_i+1}", options=pause_slots, value=curr_p, key=pk_key, disabled=is_locked
                            )

                            curr_e = st.session_state.get(e_key, def_e)
                            if curr_e not in end_slots: curr_e = end_slots[0]
                            final_stop = st.select_slider(
                                f"Bitiş {w_i+1}", options=end_slots, value=curr_e, key=e_key, disabled=is_locked
                            )
