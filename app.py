                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                if man_s:
                                    ...
                                    final_start = st.selectbox(
                                        f"Başlangıç {w_i+1}", options=start_slots, index=start_slots.index(curr_s) if curr_s in start_slots else 0, key=f"s_sel_box{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked
                                    )
                                ...
                                if man_p:
                                    ...
                                    final_pause = st.selectbox(
                                        f"Mola {w_i+1}", options=pause_slots, index=pause_slots.index(curr_p) if curr_p in pause_slots else 0, key=f"p_sel_box{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked
                                    )
                                ...
                                if man_e:
                                    ...
                                    final_stop = st.selectbox(
                                        f"Bitiş {w_i+1}", options=end_slots, index=end_slots.index(curr_e) if curr_e in end_slots else 0, key=f"e_sel_box{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked
                                    )
