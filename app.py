...
with tab_entry:
    ...
    # --- ÖNEMLİ DÜZELTME: KAYITLI VERİLERİ ÖNCEDEN OKUYUP SAYILARI BELİRLEME ---
    ...
    for day_idx, day in enumerate(days):
        ...
        # Bu gün için kaydedilmiş benzersiz projeleri bul
        ...
        with st.expander(expand_label, expanded=(len(day_entries) > 0)):
            ...
            for p_i in range(int(num_projects)):
                ...
                if proj_sel != "Seçiniz":
                    ...
                    # Bu projeye ait kayıtlı çalışanları bul
    ...
    # Otomatik ve Garantili Kayıt Butonu
    st.session_state.weekly_data[current_week_key] = {"entries": all_entries, "expenses": all_expenses}
    db_save("weekly_data", st.session_state.weekly_data)

    st.markdown("---")
    col_save_btn1, col_save_btn2 = st.columns([3, 1])
    with col_save_btn1:
        if st.button("💾 Bu Haftanın Verilerini ve Değişiklikleri Garantili Kaydet", type="primary", use_container_width=True):
            db_save("weekly_data", st.session_state.weekly_data)
            st.success("Bu haftaya ait tüm veriler, saatler, fişler ve resimler eksiksiz olarak veritabanına ve yedeğe kaydedildi!")
    with col_save_btn2:
