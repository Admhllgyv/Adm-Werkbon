...
# --- OTOMATİK HAFTALIK ARKA PLAN YEDEKLEME VE GERİ YÜKLEME SİSTEMİ ---
...
# --- UYGULAMA AÇILIRKEN OTOMATİK GERİ YÜKLEME (OTOMATİK GERİ AL / KURTARMA) ---
...
with tab_settings:
    st.header("⚙️ Ayarlar, Otomatik Yedekleme ve Geri Yükleme")
    st.info(f"📂 **Otomatik Yedekleme Klasörü:** Her Cumartesi saat 02:00'de otomatik olarak şu klasöre yedek alınır:\n`{TARGET_BACKUP_DIR}/`")
    
    col_st1, col_st2 = st.columns(2)
    with col_st1:
        if st.button("🚀 Şimdi Test Yedeği Al"):
            try:
                os.makedirs(TARGET_BACKUP_DIR, exist_ok=True)
                ...
                with open(test_path, "w", encoding="utf-8") as f:
                    json.dump(export_data, f, ensure_ascii=False)
                st.success(f"Başarılı! Test yedeği kaydedildi:\n`{test_path}`")
            except Exception as e:
                st.error(f"Hata oluştu:\n{e}")
                
    with col_st2:
        if os.path.exists(AUTO_BACKUP_FILE):
            if st.button("🔄 Son Otomatik Yedekten Geri Yükle (Geri Al)"):
                try:
                    with open(AUTO_BACKUP_FILE, "r", encoding="utf-8") as f:
                        imported_data = json.load(f)
                        ...
                        st.success("Son otomatik yedek başarıyla geri yüklendi!")
                        st.rerun()
                except Exception as e:
...
with tab_preview:
    ...
    with col_py1:
        prev_sel_year = st.number_input("Yıl", min_value=2026, max_value=2035, value=current_iso_year, step=1, key="prev_y_slide")
    with col_py2:
        prev_week_no = st.number_input("Hafta No", min_value=1, max_value=53, value=current_iso_week, step=1, key="prev_w_slide")
...
with tab_entry:
    ...
    with col_ey1:
        sel_year = st.number_input("Yıl Seçimi", min_value=2026, max_value=2035, value=current_iso_year, step=1, key="entry_y_slide")
    with col_ey2:
        week_no = st.number_input("Hafta Seçimi", min_value=1, max_value=53, value=current_iso_week, step=1, key="entry_w_slide")
    ...
    for day_idx, day in enumerate(days):
        ...
        with st.expander(expand_label, expanded=(len(day_entries) > 0)):
            ...
            for p_i in range(int(num_projects)):
                ...
                if proj_sel != "Seçiniz":
                    ...
                    for w_i in range(int(num_workers)):
                        ...
                        worker_sel = st.selectbox(f"Çalışan {w_i+1}", worker_options, key=w_key)
                        
                        col_t1, col_t2, col_t3 = st.columns(3)
                        with col_t1:
                            start_time = st.selectbox(f"Başlangıç ({worker_sel})", start_slots, index=32, key=s_key)
                        with col_t2:
                            pause_time = st.selectbox(f"Mola ({worker_sel})", pause_slots, index=2, key=pk_key)
                        with col_t3:
                            end_time = st.selectbox(f"Bitiş ({worker_sel})", end_slots, index=68, key=e_key)
                            
                        net_hours = calculate_net_hours(start_time, pause_time, end_time)
                        st.info(f"Hesaplanan Net Çalışma: **{net_hours} Saat**")
                        
                        if worker_sel != "Seçiniz":
                            all_entries.append({
                                "Gün": day,
                                "Tarih": day_date_str,
                                "Proje": proj_sel,
            ...
            # Fiş ve Gider Bölümü
            st.markdown("#### 💶 Fiş / Gider Girişi")
            num_exp_key = f"num_exp_{day}_{week_no}"
            saved_day_expenses = saved_expenses_by_day.get(day, [])
            if num_exp_key not in st.session_state:
                st.session_state[num_exp_key] = len(saved_day_expenses)
                
            num_expenses = st.number_input(f"Fiş Sayısı ({day})", min_value=0, max_value=5, key=num_exp_key)
            
            for ex_i in range(int(num_expenses)):
                ex_cat_key = f"ex_cat_{day}_{ex_i}_{week_no}"
                ex_desc_key = f"ex_desc_{day}_{ex_i}_{week_no}"
                ex_amt_key = f"ex_amt_{day}_{ex_i}_{week_no}"
                ex_img_key = f"ex_img_{day}_{ex_i}_{week_no}"
                
                ec1, ec2, ec3 = st.columns(3)
                with ec1:
                    ex_cat = st.selectbox(f"Kategori {ex_i+1}", st.session_state.expense_categories, key=ex_cat_key)
                with ec2:
                    ex_desc = st.text_input(f"Açıklama {ex_i+1}", key=ex_desc_key)
                with ec3:
                    ex_amt = st.number_input(f"Tutar (€) {ex_i+1}", min_value=0.0, value=0.0, step=0.5, key=ex_amt_key)
                    
                ex_img_file = st.file_uploader(f"Fiş Görseli {ex_i+1} ({day})", type=["jpg", "jpeg", "png"], key=ex_img_key)
                ex_img_b64 = None
                if ex_img_file is not None:
                    try:
                        ex_img_b64 = base64.b64encode(ex_img_file.getvalue()).decode('utf-8')
                    except Exception:
                        pass
                
                if ex_amt > 0 or ex_desc:
                    all_expenses.append({
                        "Gün": day,
                        "Tarih": day_date_str,
                        "Proje": "Genel",
                        ...
                        "Açıklama": ex_desc,
                        "Tutar": ex_amt,
                        "Görsel_b64": ex_img_b64
                    })

    # Haftalık veriyi güncelle ve kaydet
    ...
    with col_sv1:
        if st.button("💾 Haftalık Kaydı Kaydet ve Veritabanına İşle", type="primary"):
            db_save("weekly_data", st.session_state.weekly_data)
            st.success("Haftalık çalışma ve gider kayıtları başarıyla kaydedildi!")
            
    with col_sv2:
        is_completed = current_week_key in st.session_state.completed_weeks
        if is_completed:
            if st.button("🔴 Haftayı Tekrar Aktif Yap"):
                st.session_state.completed_weeks.remove(current_week_key)
                db_save("completed_weeks", list(st.session_state.completed_weeks))
                st.success("Hafta tekrar aktif hale getirildi.")
                st.rerun()
        else:
            if st.button("🟢 Haftayı Tamamla / Kilitle"):
                st.session_state.completed_weeks.add(current_week_key)
                db_save("completed_weeks", list(st.session_state.completed_weeks))
                st.success("Hafta başarıyla tamamlandı.")
                st.rerun()
