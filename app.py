...
# Session State Senkronizasyonu (Doğru varsayılan değerlerle)
...
# PDF Önizleme Yardımcı Fonksiyonu
def render_pdf_preview(pdf_bytes, expander_label="🔍 PDF Önizle"):
...
def create_monthly_pdf(year, month_num, month_name, worker_filter, entries, expenses, hourly_rates):
    buffer = io.BytesIO()
    ...
    filter_txt = worker_filter if worker_filter != "Tümü" else "Tüm Çalışanlar"
    header_data = [[header_left_elements, Paragraph(f"<b>Aylık Özet Raporu</b><br/><b>Dönem:</b> {month_name} {year}<br/><b>Çalışan:</b> {filter_txt}", meta_p_style)]]
    ...
    if entries:
        story.append(Paragraph("<b>Çalışan Hak Ediş ve Saat Özeti:</b>", ParagraphStyle('SubT', fontName='Helvetica-Bold', fontSize=9, textColor=primary_color)))
        ...
        story.append(Paragraph("<b>Detaylı Çalışma Kayıtları:</b>", ParagraphStyle('SubT2', fontName='Helvetica-Bold', fontSize=9, textColor=primary_color)))
        story.append(Spacer(1, 4))
        ...
        for e in entries:
            detail_table_data.append([
                Paragraph(f"{e.get('Gün', '')} ({e.get('Tarih', '')})", sub_style),
...
with tab_monthly:
    ...
    with m_col1:
        sel_m_year = st.selectbox("Yıl", [2026, 2027, 2028, 2029, 2030], index=0, key="monthly_year")
    with m_col2:
        months_dict = {
        ...
        sel_m_month_num = st.selectbox("Ay", options=list(months_dict.keys()), format_func=lambda x: months_dict[x], index=current_m - 1, key="monthly_month")
    with m_col3:
    ...
    if monthly_entries:
        ...
        st.subheader(f"⏱️ {months_dict[sel_m_month_num]} {sel_m_year} Çalışma Saatleri ve Hak Ediş Özeti")
        ...
        m_pdf_file = create_monthly_pdf(sel_m_year, sel_m_month_num, months_dict[sel_m_month_num], sel_m_worker, monthly_entries, monthly_expenses, st.session_state.worker_hourly_rates)
        m_pdf_bytes = m_pdf_file.getvalue()
        
        worker_filename_part = sel_m_worker if sel_m_worker != "Tümü" else "Tum_Calisanlar"
        monthly_pdf_filename = f"{months_dict[sel_m_month_num]}_{sel_m_year}_{worker_filename_part}_Aylik_Ozet.pdf"
        
        st.session_state.pdf_archive[monthly_pdf_filename] = base64.b64encode(m_pdf_bytes).decode('utf-8')
        db_save("pdf_archive", st.session_state.pdf_archive)

        # Önizleme ve İndirme butonları yan yana
        col_m_dl1, col_m_dl2 = st.columns([1, 1])
        with col_m_dl1:
            st.download_button(
                label=f"📥 {months_dict[sel_m_month_num]} {sel_m_year} Aylık Raporu İndir",
                data=m_pdf_bytes,
        ...
        with col_m_dl2:
            if st.button("🔍 Aylık Raporu Önizle", key="btn_prev_monthly"):
                st.session_state['show_monthly_preview'] = not st.session_state.get('show_monthly_preview', False)
        
        if st.session_state.get('show_monthly_preview', False):
            render_pdf_preview(m_pdf_bytes, expander_label="📄 Aylık Rapor PDF Görüntüleyicisi")
            
    else:
        st.info(f"{months_dict[sel_m_month_num]} {sel_m_year} dönemi için kayıt bulunamadı.")
...
with tab_archive:
    ...
    if st.session_state.pdf_archive:
        ...
        for filename, b64_pdf in list(clean_archive.items()):
            ...
            col_ar1, col_ar2, col_ar3, col_ar4 = st.columns([2.5, 1, 1, 1])
            with col_ar1:
                st.write(f"📄 {filename}")
            with col_ar2:
                try:
                    ...
                    pass
            with col_ar3:
                if st.button("🔍 Önizle", key=f"arch_prev_btn_{filename}"):
                    st.session_state[f"show_prev_{filename}"] = not st.session_state.get(f"show_prev_{filename}", False)
            with col_ar4:
            ...
            if st.session_state.get(f"show_prev_{filename}", False):
                render_pdf_preview(pdf_bytes, expander_label=f"📄 Önizleme: {filename}")
            st.markdown("---")
    else:
        st.info("Arşiv boş.")
...
# --- 📝 WERKBON SEKMESİ (BÜTÜN İŞLEMLER VE ÖNİZLEME BUTONLARI BURADA) ---
with tab_entry:
                ...
                for p_i in range(int(num_projects)):
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            wrk = st.selectbox(f"Çalışan {w_i+1} ({proj_sel})", worker_options, key=w_key, disabled=is_week_locked)
                            ...
                            if wrk != "Seçiniz":
                                ...
                                with col_s1:
                                    st.markdown(f"Başlangıç")
                                with col_s2:
                                    man_s = st.checkbox("Manuel", key=f"man_s_{day}{p_i}{w_i}_{week_no}", disabled=is_week_locked)

                                if man_s:
                                    final_start = st.text_input("Manuel Başlangıç", value=st.session_state.get(s_key, def_s), key=f"s_man_{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked)
                                else:
                                    curr_s = st.session_state.get(s_key, def_s)
                                    if curr_s not in start_slots:
                                        curr_s = start_slots[0]
                                    final_start = st.select_slider(
                                        f"Başlangıç {w_i+1}", options=start_slots, value=curr_s, key=f"s_sl{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked
                                    )
                                ...
                                with col_p1:
                                    st.markdown(f"Mola")
                                with col_p2:
                                    man_p = st.checkbox("Manuel", key=f"man_p_{day}{p_i}{w_i}_{week_no}", disabled=is_week_locked)

                                if man_p:
                                    final_pause = st.text_input("Manuel Mola", value=st.session_state.get(pk_key, def_p), key=f"p_man_{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked)
                                else:
                                    curr_p = st.session_state.get(pk_key, def_p)
                                    if curr_p not in pause_slots:
                                        curr_p = pause_slots[0]
                                    final_pause = st.select_slider(
                                        f"Mola {w_i+1}", options=pause_slots, value=curr_p, key=f"p_sl{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked
                                    )
                                ...
                                with col_e1:
                                    st.markdown(f"Bitiş")
                                with col_e2:
                                    man_e = st.checkbox("Manuel", key=f"man_e_{day}{p_i}{w_i}_{week_no}", disabled=is_week_locked)

                                if man_e:
                                    final_stop = st.text_input("Manuel Bitiş", value=st.session_state.get(e_key, def_e), key=f"e_man_{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked)
                                else:
                                    curr_e = st.session_state.get(e_key, def_e)
                                    if curr_e not in end_slots:
                                        curr_e = end_slots[0]
                                    final_stop = st.select_slider(
                                        f"Bitiş {w_i+1}", options=end_slots, value=curr_e, key=f"e_sl{day}{p_i}{w_i}_{week_no}", label_visibility="collapsed", disabled=is_week_locked
                                    )
    ...
    if active_projects:
        st.markdown("#### Aktif Proje PDF İndirme ve Önizleme Bağlantıları:")
        for proj in active_projects:
            pdf_file = create_pdf(proj, all_entries, all_expenses, week_no, sel_year)
            pdf_bytes = pdf_file.getvalue()
            clean_proj_name = proj.replace(' ', '_').replace('(', '').replace(')', '')
            ...
            with col_p_btn2:
                if st.button(f"🔍 Önizle: {proj}", key=f"preview_btn_{proj}{week_no}"):
                    st.session_state[f"show_wb_prev_{proj}{week_no}"] = not st.session_state.get(f"show_wb_prev_{proj}{week_no}", False)
            
            if st.session_state.get(f"show_wb_prev_{proj}{week_no}", False):
                render_pdf_preview(pdf_bytes, expander_label=f"📄 Önizleme: {proj} (Week {week_no})")
            st.markdown("---")
    else:
