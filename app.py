...
<script>
// Seçili olan saat kutucuğunu şeridin TAM ORTASINA kilitleyen kusursuz motor
function centerSelectedTimeChip() {
...
// Streamlit'in her render/rerun döngüsünde anında tetikle
[50, 150, 300, 600, 1000].forEach(delay => {
    setTimeout(centerSelectedTimeChip, delay);
});

// Sayfa içeriği değiştiğinde (seçim yapıldığında) anında ortala
const observer = new MutationObserver(() => {
    centerSelectedTimeChip();
});
if (document.body) {
...
document.addEventListener('change', (e) => {
    if (e.target && e.target.type === 'radio') {
        setTimeout(centerSelectedTimeChip, 50);
        setTimeout(centerSelectedTimeChip, 250);
    }
});
</script>
