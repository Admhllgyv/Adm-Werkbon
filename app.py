...
# --- MANUEL SAAT SEÇİCİ (TELEFONA UYUMLU DROPDOWN) ---
def time_picker_manual_component(label, min_h, max_h, session_key, default_val, disabled=False):
    ...
    col_h, col_m, col_info = st.columns([1.5, 1.5, 2])
    
    hours = list(range(min_h, max_h + 1))
    h_key = f"manual_h_{session_key}"
    if h_key not in st.session_state or st.session_state[h_key] not in hours:
        st.session_state[h_key] = def_h
        
    minutes = [0, 15, 30, 45]
    m_key = f"manual_m_{session_key}"
    if m_key not in st.session_state or st.session_state[m_key] not in minutes:
        st.session_state[m_key] = def_m
        
    with col_h:
        sel_h = st.selectbox("Saat", hours, key=h_key, disabled=disabled, label_visibility="collapsed")
    with col_m:
        sel_m = st.selectbox("Dakika", minutes, key=m_key, disabled=disabled, label_visibility="collapsed", format_func=lambda x: f"{x:02d}")
    with col_info:
        st.markdown(f"<div style='padding-top: 6px; font-size: 12px; font-weight: bold; color: #2563EB;'>Seçilen: {sel_h:02d}:{sel_m:02d}</div>", unsafe_allow_html=True)
        
    return f"{sel_h:02d}:{sel_m:02d}"

def break_picker_manual_component(label, session_key, default_val, disabled=False):
    st.markdown(f"<span style='font-size:11px; font-weight:bold; color:#1F4E78;'>{label}</span>", unsafe_allow_html=True)
    
    break_options = ["00:00", "00:15", "00:30", "00:45", "01:00", "01:15", "01:30", "01:45", "02:00"]
    b_key = f"b_manual_{session_key}"
    ...
    with col2:
        st.markdown(f"<div style='padding-top: 6px; font-size: 12px; font-weight: bold; color: #2563EB;'>Mola: {sel_break}</div>", unsafe_allow_html=True)
                    ...
                    if proj_sel != "Seçiniz":
                        ...
                        for w_i in range(int(num_workers)):
                            ...
                            if wrk != "Seçiniz":
                                ...
                                final_start = time_picker_manual_component("Başlangıç Saati", 3, 12, f"s_{day}{p_i}{w_i}_{week_no}", st.session_state.get(s_key, def_s), disabled=is_week_locked)
                                st.session_state[s_key] = final_start

                                final_pause = break_picker_manual_component("Mola Süresi", f"p_{day}{p_i}{w_i}_{week_no}", st.session_state.get(pk_key, def_p), disabled=is_week_locked)
                                st.session_state[pk_key] = final_pause

                                final_stop = time_picker_manual_component("Bitiş Saati", 11, 20, f"e_{day}{p_i}{w_i}_{week_no}", st.session_state.get(e_key, def_e), disabled=is_week_locked)
